...
/*
 * Squelch any normal output from the called function. This is used
 * during headless init to reduce scrambled screen clutter.
 */

#define DIAG_FLAG_NOOUTPUT	0x100
...
#define KLDIAG_MEMBIST_FAILED   48      /* Failed memory bist for some bank */

/* IO6 error codes */
#define KLDIAG_XBOW_FAIL           50  /* XBOW sanity on IO6 failed */
#define KLDIAG_BRIDGE_FAIL         51  /* Bridge sanity on IO6 failed */
#define KLDIAG_IOC_BASE_FAIL       52  /* IOC3 or IOC4 base address not found */
...
#define KLDIAG_SER_PIO_FAIL1C      57  /* Chars already exist in port C. */
#define KLDIAG_SER_PIO_FAIL1D      58  /* Chars already exist in port D. */
#define KLDIAG_SER_PIO_FAIL2A      59  /* Miscompares on A. */
#define KLDIAG_SER_PIO_FAIL2B      60  /* Miscompares on B. */
#define KLDIAG_SER_PIO_FAIL2C      61  /* Miscompares on C. */
#define KLDIAG_SER_PIO_FAIL2D      62  /* Miscompares on D. */
...
#define KLDIAG_PCKM_KBD_FAIL       73  /* Keyboard test failed */
#define KLDIAG_PCKM_MSE_FAIL       74  /* Mouse test failed */
#define KLDIAG_SER_DMA_FAILA       75  /* Miscompares on A in dma mode. */
#define KLDIAG_SER_DMA_FAILB       76  /* Miscompares on B in dma mode. */
#define KLDIAG_SER_DMA_FAILC       77  /* Miscompares on C in dma mode. */
#define KLDIAG_SER_DMA_FAILD       78  /* Miscompares on D in dma mode. */
...
#define KLDIAG_PCKM_EXCEPTION      89
...
#define KLDIAG_GFX_EXCEPTION       130 
#define KLDIAG_GFX_INFO_FAIL       131 
#define KLDIAG_GFX_XTALK_FAIL      132 
#define KLDIAG_GFX_DCB_FAIL        133 
#define KLDIAG_GFX_BUZZ_FAIL       134 
#define KLDIAG_GFX_ANALOG_FAIL     135 
...
#define KLDIAG_CONFLICT         254     /* Some devices are in conflict */
...
/*
 * diagnostic error message format for slot/widget path
 */
#ifndef SN1
# define DGERRFMT "/hw/module/%M/slot/io%d"
# define GET_MODULE_SLOTWID(x, y, z) get_module_slot(x, y, z)
#else
# define DGERRFMT "/hw/module/%M/xtalk/%d"
# define GET_MODULE_SLOTWID(x, y, z) get_module_widgetnum(x, y, z)
#endif /* SN1 */
...
extern int diag_pckm(int             diag_mode,
                     __psunsigned_t  bridge_baseAddr,
                     int             npci,
                     char *          env_console);

extern int diag_gfx(int diag_mode, int type, int level);
