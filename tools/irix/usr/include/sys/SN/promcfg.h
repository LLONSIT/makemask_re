...
#else
#define PCFG_HUB_PEBRICK	(1 << 17)	/* This is a PE-brick	     */
#endif
...
#define PCFG_ROUTER_REPEATER     (1 <<  1)	/* Is it a repeater-router?  */
#define PCFG_ROUTER_REMOTE_SPACE (1 <<  2)	/* Not in reset partition    */
#define PCFG_ROUTER_CLOCK        (1 <<  3)	/* can supply partition clock*/
...
#define MAX_FTDEPTH		0x7U
#define PCFG_ROUTER_FTDEPTH_SHFT 12
#define PCFG_ROUTER_FTDEPTH_MASK (MAX_FTDEPTH << PCFG_ROUTER_FTDEPTH_SHFT)
...
#ifdef SN1
typedef	__uint64_t	bar_cfg_t;

typedef struct bar_ent_s {
	net_vec_t	vec;		/* vector route to router in tree */
	bar_cfg_t	cfg;		/* barrier config register settings */
} bar_ent_t;
#endif
...
#else
	char		unused1[1];	/* Space wasted due to alignment     */
	moduleid_t	io_module;	/* I/O Brick Module ID, if connected */
	moduleid_t	module;		/* Module ID			     */
#endif
...
#else
	uchar_t		unused;		/* Unused			     */
#endif
	ushort		flags;		/* PCFG_ROUTER_xxx flags	     */
	nic_t		nic;		/* Universally unique ID no.         */
	pcfg_port_t	port[MAX_ROUTER_PORTS + 1];	/* Port devices 1 to 6/8 (zero unused) */
	char		portstat1;	/* Basic diag status of ports 1 to 6/8 */
...
#else
	char		padsize[1024];	/* Pad entry size to power of 2      */
#endif
} pcfg_ent_t;
...
#define PCFG_K1(nasid)		((pcfg_t *) TO_NODE(nasid, IP27PROM_PCFG_K1))
#define PCFG_PTR(nasid, i)	(&PCFG(nasid)->array[i])
#define PCFG_PTR_K1(nasid, i)	(&PCFG_K1(nasid)->array[i])
