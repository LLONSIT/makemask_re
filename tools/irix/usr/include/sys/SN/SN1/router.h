...
#ifndef	__SYS_SN_SN1_ROUTER_H__
#define	__SYS_SN_SN1_ROUTER_H__
...
#ifdef VECTOR_JTAG_WAR
typedef struct router_port_info_sup_s {
	router_count_t	rp_retry_last;		/* last retry value */
	router_count_t	rp_sn_last;		/* last sn value */
	router_count_t	rp_cb_last;		/* last cb value */
} router_port_info_sup_t;
#endif /* VECTOR_JTAG_WAR */
...
typedef struct router_info_s {
	...
	char *		ri_name;	/* This board's hwg path 	    */
        unsigned char	ri_port_maint[MAX_ROUTER_PORTS]; /* should we send a 
					message to availmon */
#ifdef VECTOR_JTAG_WAR
	router_port_info_sup_t ri_port_sup[MAX_ROUTER_PORTS]; /* add. info  */
#endif /* VECTOR_JTAG_WAR */
} router_info_t;
