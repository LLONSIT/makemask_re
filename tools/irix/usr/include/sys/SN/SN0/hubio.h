...
#ifndef	__SYS_SN_SN0_HUBIO_H__
#define	__SYS_SN_SN0_HUBIO_H__
...
typedef union hubii_wid_u {
	__uint64_t	wid_reg_value;
	struct {
	    __uint64_t 	wid_rsvd: 	32,	/* unused */
...
typedef union hubii_wcr_u {
	__uint64_t	wcr_reg_value;
	struct {
	    __uint64_t 	wcr_rsvd: 	41,	/* unused */
...
typedef union hubii_ilcsr_u {
	__uint64_t	icsr_reg_value;
	struct {
	    __uint64_t 	icsr_rsvd: 	22,	/* unused */
...
typedef union hubii_iowa_u {
	__uint64_t	iowa_reg_value;
	struct {
	    __uint64_t 	iowa_rsvd: 	48,	/* unused */
...
typedef union hubii_iiwa_u {
	__uint64_t	iiwa_reg_value;
	struct {
	    __uint64_t 	iiwa_rsvd: 	48,	/* unused */
...
typedef union	hubii_illr_u {
	__uint64_t	illr_reg_value;
	struct {
	    __uint64_t 	illr_rsvd: 	32,	/* unused */
...
/*
 * The bridge while issuing write requests to the xtalk
 * for the PCI devices marks them as 0x10 + pci device
 * number. Basically the valid values in the case
 * are 0x10-0x17.
 */
#define BRIDGE_DMA_WRITE_REQ_TNUM_MIN	0x10
#define BRIDGE_DMA_WRITE_REQ_TNUM_MAX	0x17
#define TNUM_TO_WIDGET_DEV(_tnum)	(_tnum - BRIDGE_DMA_WRITE_REQ_TNUM_MIN)
...
typedef union icrbb_u {
	...
	struct {
	    __uint64_t	rsvd1:	5,
			...
			initiator: 3,	/* Initiator of original request 
					 * See below for field values.
					 */
...
typedef union hubii_ifdr_u {
	__uint64_t	hi_ifdr_value;
	struct {
		__uint64_t	ifdr_rsvd:	49,
		                ifdr_maxrp:	 7,
	                        ifdr_rsvd1:	 1,
                            	ifdr_maxrq:	 7;
	} hi_ifdr_fields;
...
typedef union iprte_a {
	__uint64_t	entry;
	struct {
	    __uint64_t 	rsvd1     : 7,  /* Reserved field 		*/
...
typedef union icrbp_a {
	__uint64_t   ip_reg;	    /* the entire register value	*/
	struct {
	     __uint64_t error:	1,  /*    63, error occurred		*/
 ...
 * with a Hub (depth of Hub's queue).
 */
#define HUB_CREDIT 4
...
extern cpuid_t
hub_intr_cpuid_get(hub_intr_t intr_hdl);
...
extern void hub_widgetdev_enable(vertex_hdl_t, int);
extern void hub_widgetdev_shutdown(vertex_hdl_t, int);
extern int  hub_dma_enabled(vertex_hdl_t);
extern int  is_widget_pio_enabled(ioerror_t *);
/* 
