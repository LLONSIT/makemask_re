...
#ifndef	__SYS_SN_SN0_HUBMD_H__
#define	__SYS_SN_SN0_HUBMD_H__
...
#include	<sys/R10k.h>		/* for CACHE_SLINE_SIZE definition */
...
#define MD_PTR_NODE(x)		((x) >> 2)
#define MD_PTR_DEVICE(x)	((x) & 0x3)
#define MD_PTR_SLICE(x)		((x) & 0x1)
 ...
 *  #ifdefs for SN0 and SN1 in memerror.c code.  See SN1/hubmd_next.h
 *  for corresponding SN0 definitions.
 */
#define DIR_ERR_CE_VALID(err)	(err.derr_fmt.ce_vld)
#define DIR_ERR_UCE_VALID(err)	(err.derr_fmt.uce_vld)
#define DIR_ERR_AE_VALID(err)	(err.derr_fmt.ae_vld)
#define DIR_ERR_BAD_SYN(err)	(err.derr_fmt.bad_syn)
#define DIR_ERR_CE_OVERRUN(err)	(err.derr_fmt.ce_ovr)
#define MEM_ERR_CE_VALID(err)	(err.merr_fmt.ce_vld)
#define MEM_ERR_UCE_VALID(err)	(err.merr_fmt.uce_vld)
#define MEM_ERR_ADDRESS(err)	(err.merr_fmt.address)
#define MEM_ERR_CE_OVERRUN(err)	(err.merr_fmt.ce_ovr)
#define MEM_ERR_BAD_SYN(err)	(err.merr_fmt.bad_syn)
...
struct dir_error_reg {
	__uint64_t	uce_vld:   1,	/*    63: valid directory uce 	*/
...
typedef union md_dir_error {
	__uint64_t	derr_reg;	/* the entire register 		*/
	struct dir_error_reg derr_fmt;	/* the register format		*/
} md_dir_error_t;


struct mem_error_reg {
	__uint64_t	uce_vld:   1,	/*    63: valid memory uce 	*/
...
typedef union md_mem_error {
	__uint64_t	merr_reg;	/* the entire register 		*/
	struct mem_error_reg  merr_fmt; /* format of the mem_error reg  */
} md_mem_error_t;


struct proto_error_reg {
	__uint64_t	valid:	   1,	/*    63: valid protocol error	*/
...
typedef union md_proto_error {
	__uint64_t	perr_reg;	/* the entire register 		*/
