...
#ifndef	__SYS_SN_KLCONFIG_H__
#define	__SYS_SN_KLCONFIG_H__
...
#ifdef FRUTEST
typedef __uint64_t klconf_off_t;
#else
typedef __int32_t klconf_off_t;
#endif
...
#ifdef _STANDALONE
#ifdef MAX_MODULES		/* Kernel already uses MAX_MODULES for */
#undef MAX_MODULES		/*  something different (use MODULE_MAX) */
#endif
#if defined(SN1)
#define MAX_MODULES    		128 /* 512p */
#elif defined(SN0XXL)
#define MAX_MODULES    		96 /* 512p */  /* 80 is real value, 96 works */
#else
#define MAX_MODULES    		32 /* 128p */
#endif
#endif /* _STANDALONE */
...
#if defined(SN1)
/* 
 * (1 NODE brick, 6 XIO Widgets, 1 Xbow) * 5 (speedo2 + 4 PEBricks)
 * + 3 Router bricks (1 local, 1 meta, 1 repeater),
 * + 1 gfx
 */
#define PE_MAX_SLOTS_PER_NODE	(((1 + 6 + 1) * 5) + 3 + 1) 
...
#define IS_CONSOLE_IOC(i)	((((klinfo_t *)i)->flags) & KLINFO_INSTALL)
...
typedef struct kl_config_hdr {
	__uint64_t	ch_magic;	/* set this to KLCFGINFO_MAGIC */
	__uint32_t	ch_version ;    /* structure version number */
...
#define OLD_KL_CONFIG_INFO(_nasid) 					\
        (lboard_t *)((KL_CONFIG_HDR(_nasid)->ch_board_info) ?		\
	 NODE_OFFSET_TO_K1((_nasid), KL_CONFIG_HDR(_nasid)->ch_board_info) : \
	 NULL)

#ifdef _STANDALONE
#define KL_CONFIG_INFO(_nasid) OLD_KL_CONFIG_INFO(_nasid)
#else
#  if	SN1
#define KL_CONFIG_INFO(_nasid)	(PEBRICK_NODE(_nasid) ? \
		(lboard_t *)chk_for_pebrick(_nasid) : OLD_KL_CONFIG_INFO(_nasid))
#  else /* !SN1 */
#define KL_CONFIG_INFO(_nasid) OLD_KL_CONFIG_INFO(_nasid)
#  endif /* SN1 */
#endif
...
#define KLCLASS_IOBRICK	0x70		/* SN1 iobrick */

#define KLCLASS_IO2	0x80		/* Second class of IO boards.  This
					 * is primarily used to avoid prom 
					 * warning messages.
					 */
...
#define KLTYPE_DIVO     (KLCLASS_IO2  | 0x1) /* DIVO */
#define KLTYPE_TPU    	(KLCLASS_IO2  | 0x2) /* Tensor Processing Unit */
#define KLTYPE_GSN_A   	(KLCLASS_IO2  | 0x3) /* Main GSN board */
#define KLTYPE_GSN_B   	(KLCLASS_IO2  | 0x4) /* Auxiliary GSN board */
#define KLTYPE_SHOEHORN (KLCLASS_IO2  | 0x5)
#define KLTYPE_SERIAL_HIPPI (KLCLASS_IO2  | 0x6)
#define KLTYPE_QUAD_ATM (KLCLASS_IO2  | 0x7)

#define KLTYPE_GFX	(KLCLASS_GFX | 0x0) /* unknown graphics type */
#define KLTYPE_GFX_KONA (KLCLASS_GFX | 0x1) /* KONA graphics on SN0/SN1 */
#define KLTYPE_GFX_MGRA (KLCLASS_GFX | 0x3) /* MGRAS graphics on SN0 */
#define KLTYPE_GFX_ODSY (KLCLASS_GFX | 0x4) /* ODSY graphics on SN1 */
...
#endif
	unsigned short	brd_bus_num : 2,  /* Used to uniquely identify PIC PCI Bus 1 */
			brd_unused  : 14; /* Reserved for potential use later */
...
/*
 * PIC bus 1 needs to be uniquely identified in klinfo_s.physid so
 * that we can make a system wide unique number in the prom (see
 * get_locator_id()).  physid is also used in other code where it is assumed
 * to be equivalent to widgetid.   Hide all this in these macros.
 */
#define KLCF_PIC_BUS1_MARKER	0x10
#define KLCF_BRIDGE_W_ID(_comp)	(((_comp)->physid) & ~KLCF_PIC_BUS1_MARKER) /* Widget ID */
#define KLCF_LOCATOR_SHIFT 5
...
typedef struct klinfo_s {                         /* Generic info */
	...
	unsigned short	port : 2,	   /* changed pad1 to support dual port ctlrs. */
			pci_bus_num : 2,   /* PCI bus number per PIC widget */
			pci_is_multifunc : 1,	/* device is multifunction */
			pci_func_num : 3,  /* function number */
			pci_unused  : 8;
...
typedef __uint64_t *pci_t ;
typedef __uint64_t *vmeb_t ;
typedef __uint64_t *vmed_t ;
typedef __uint64_t *fddi_t ;
typedef __uint64_t *scsi_t ;
typedef __uint64_t *mio_t ;
typedef __uint64_t *graphics_t ;
typedef __uint64_t *router_t ;
...
typedef struct klcpu_s {                          /* CPU */
	...
	unsigned short 	cpu_prid ;         /* Processor PRID value */
        ...
        /*
         * was FPU IRR which is not defined for on-chip FPU
         * now used to store some other important mode bit
         * information which can be reported by hinv and others
         */
        unsigned short  cpu_mode;
...
typedef unsigned char dist_vec[MAX_NASIDS/2]; /* nibbles */

#ifdef SN1
#define HUB_STRUCT_VERSION      2
#else /* !SN1 */
#define HUB_STRUCT_VERSION	1
#endif /* !SN1 */
...
typedef struct klhub_s {                          /* HUB */
	...
	__uint64_t	hub_speed ;        /* Speed of hub in HZ */
#ifdef SN1
	dist_vec	dist ;             /* distances to other hubs */
#endif /* SN1 */
} klhub_t ;
...
typedef struct klmembnk_s {                          /* MEMORY BANK */
	...
	short		membnk_attr; 		/* directory flavor */
} klmembnk_t ;
...
typedef struct klpci_device_s {
	...
	__int32_t	pci_device_id;	/* 32 bits of vendor/device ID. */

        union {         /* 32bits pci class/subclass/interface */
        __int32_t       pci_cl_rev;
        __int32_t       pci_class:8, pci_subclass:8, pci_pgm_if:8, pci_rev_id:8;
        } pci_cr;
...
typedef struct klioc_s {                          /* Common IOC3/IOC4 */
	klinfo_t 	ioc_info ;
    	unsigned char	ioc_ssram ;        /* Info about ssram */
    	unsigned char	ioc_nvram ;        /* Info about nvram */
	klconf_off_t	ioc_tty_off ;
	klconf_off_t	ioc_kbd_off ;
} klioc_t ;

#define MAX_IOC3_TTY	2
#define ioc3_info       _ioc3_info.ioc_info
#define ioc3_ssram      _ioc3_info.ioc_ssram
#define ioc3_nvram      _ioc3_info.ioc_nvram
#define ioc3_tty_off    _ioc3_info.ioc_tty_off
#define ioc3_kbd_off    _ioc3_info.ioc_kbd_off
...
#define MAX_IOC4_TTY	4
#define ioc4_info       _ioc4_info.ioc_info

typedef struct klioc4_s {                          /* IOC4 */
	klioc_t 	_ioc4_info ;
    	klinfo_t	ioc4_uarts ;        /* Info about UARTs */
} klioc4_t ;

typedef struct klgige_s {                          /* TIGON3 GIGE */
        klinfo_t        gige_info ;
        unsigned char   gige_ssram ;        /* Info about ssram */
        unsigned char   gige_nvram ;        /* Info about nvram */
} klgige_t ;

typedef struct klvgagfx_s {                          /* generic PCI VGA */
        klinfo_t        vga_info ;

        struct klvgagfx_s  *vga_next_pipe;
        __psunsigned_t  hw_base; /* raw bridge-base */
        lboard_t        *bridge_lboard; /* lboard of bridge */

        /* stuff which at kernel earlyinit must be known */
        cnodeid_t       cnode_id;  /* cnode id */
        unsigned char   pci_slot;
        unsigned char   pci_is_multifunc;
        unsigned char   func_num;
        unsigned char   is_pic;
        unsigned char   pic_bus;
        unsigned char   is_tport;
        unsigned char   br_devnum; /* bridge devio(x) index */

        __psunsigned_t  cfg_base; /* cfg-space */

        /* and for running early in the kernel we need: */
        __psunsigned_t  legacy_io_base; /* legacy io */
        __psunsigned_t  legacy_device_x_reg; /* bridge ctl reg */
        __psunsigned_t  lfb_base; /* linear fb */
        __psunsigned_t  lfb_device_x_reg; /* bridge ctl reg */

        /* and the mode/res it's currently in */
        unsigned short  x,y;
        unsigned short  mode;
...
typedef struct klrou_s {                          /* ROUTER */
	...
	nic_t		rou_box_nic ;         /* nic of the containing module */
    	klport_t 	rou_port[MAX_ROUTER_PORTS + 1] ; /* array index 1 to MAX_ROUTER_PORTS */
	klconf_off_t	rou_mfg_nic ;     /* MFG NIC string */
	__uint64_t	rou_vector;	  /* vector from master node */
} klrou_t ;
...
typedef struct klpebrick_s {				/* PEBrick */
	klinfo_t	pebrick_info;
	nasid_t		host_nasid;	/* nasid where the klconfig is hosted */
	nasid_t		pe_nasid;	/* nasid of the PEBrick itself */
	vertex_hdl_t	node_vhdl;
...
typedef union klcomp_s {
	...
	klusb_t		kc_usb;
	klpebrick_t	kc_pebrick;
	klvgagfx_t      kc_vgagfx;
} klcomp_t;
...
typedef struct kl_pebricks_s {
	int	valid;
	nasid_t	pe_nasid;
	nasid_t	host_nasid;
} kl_pebricks_t;

lboard_t * chk_for_pebrick(nasid_t);

/* external declarations */

#ifdef _STANDALONE
int printf(const char *, ...) ;
extern int get_nasid_slice(cpuid_t cpu, nasid_t *nasid, int *slice);
extern int get_cpu_slice(cpuid_t);
extern void get_cpu_cnode_slice(cpuid_t, cnodeid_t *, int *);
#else
...
extern lboard_t *find_lboard(lboard_t *start, unsigned char brd_type);
extern lboard_t *find_lboard_class(lboard_t *start, unsigned char brd_class);
extern lboard_t *find_lboard_slot_class(lboard_t *start, unsigned char brd_class, slotid_t slot);
...
extern klinfo_t *find_first_component(lboard_t *brd, unsigned char struct_type);
...
extern void	board_to_full_path(lboard_t *brd, char *path);
...
extern nasid_t  get_xbow_peer(nasid_t);
extern net_vec_t klcfg_discover_route(lboard_t *, lboard_t *, int);
#endif /* __SYS_SN_KLCONFIG_H__ */
