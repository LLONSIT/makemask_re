...
#ifdef SN1
	hubreg_t	hp_spurious_hdr0; /* spurious message header */
	hubreg_t	hp_spurious_hdr1; /* spurious message header */
#endif /* SN1 */
...
#ifdef SN1
	hubreg_t	hm_misc1_err; 	  /* hub md misc1_error reg */
#endif /* SN1 */
...
#ifdef SN1
	hubreg_t	hi_ieph1;	/* incoming error packet header */
	hubreg_t	hi_ieph2;	/* incoming error packet header */
#endif
...
#ifdef SN0			/* vector status is in LB on SN1 */
	hubreg_t	hn_vec_sts;	/* hub ni vector status */
#endif
	hubreg_t	hn_port_err;	/* hub ni port errors register */
#ifdef SN1
	hubreg_t	hn_headerA;	/* errored packet header, part A */
	hubreg_t	hn_headerB;	/* errored packet header, part B */
	hubreg_t	hn_sideband;	/* errored packet sideband */
#endif
...
typedef struct hub_lberr_s {
	klerr_info_t	hl_err_info;
	hubreg_t	hl_err_bits;	/* bedrock lb error status */
	hubreg_t	hl_err_hdr1;	/* errored packet header 1 */
	hubreg_t	hl_err_hdr2;	/* errored packet header 2 */
	hubreg_t	hl_err_data;	/* errored packet data flit */
	hubreg_t	hl_vec_sts;	/* bedrock lb vector status */
	confidence_t	hl_confidence;	/* confidence that lb is bad */
} hub_lberr_t;
...
typedef struct hub_xberr_s {
	klerr_info_t	hx_err_info;
	hubreg_t	hx_first_error;	/* bedrock xb error status */
	/* Should we save all 14 queue error regs. instead of just the one? */
	hubreg_t	hx_err_detail;	/* the errored queue's status reg. */
	confidence_t	hx_confidence;	/* confidence that xb is bad */
} hub_xberr_t;
#endif /* SN1 */
...
#ifdef SN1
	hub_pierr_t	hb_pi2;		/* info on bedrock 2nd pi errors */
	hub_lberr_t	hb_lb;		/* info on bedrock lb errors */
	hub_xberr_t	hb_xb;		/* info on bedrock xb errors */
#endif
...
typedef struct klhub_err_ext_s {
	...
	hubreg_t	hi_prb_ent[IIO_NUM_IPRBS]; /* prbs */
...
#else
	__uint64_t	br_int_status;

	__uint32_t	br_is_pic;	/* is a PIC ASIC */
	__uint32_t	br_is_pcix;	/* bus mode pci/pcix */

	bridgereg_t	br_arbitration; /* Arbitration Error */
	picreg_t	br_link_side;	/* Address holding linkside err */
	picreg_t	br_ram_parity;	/* Internal RAM parity error */
	picreg_t	br_dma_req_addr;/* PCI-X DMA request error addr */
	picreg_t	br_dma_req_attr;/* PCI-X DMA request error attribute */
	picreg_t	br_pio_req_addr;/* PCI-X PIO split request addr */
	picreg_t	br_pio_req_attr;/* PCI-X PIO split request attribute */
	picreg_t	br_bus_err_addr;/* PCI-X Bus error addr */
	picreg_t	br_bus_err_attr;/* PCI-X Bus error attribute */
	picreg_t	br_bus_err_data;/* PCI-X Bus error data */
#endif
} bridge_error_t;
...
typedef struct klrouter_err_s {
	...
	hubreg_t	re_status_error[MAX_ROUTER_PORTS];	
...
void save_router_err(nasid_t, net_vec_t, klrouter_err_t *);
...
#ifdef SN1
void save_hub_lb_error(__psunsigned_t, hub_lberr_t *);
void save_hub_xb_error(__psunsigned_t, hub_xberr_t *);
#endif
...
#ifdef SN1
void show_hub_pi_err(hub_pierr_t *, nasid_t, int); /* bedrock has two PIs */
void show_hub_lb_err(hub_lberr_t *);
void show_hub_xb_err(hub_xberr_t *);
#else
