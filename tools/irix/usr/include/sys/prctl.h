/**************************************************************************
 *									  *
 * 		 Copyright (C) 1986-2004 Silicon Graphics, Inc.		  *
 *									  *
 ...
 *
 * IMPORTANT: Whenever a change is made to prda_sys, make sure to run
...
struct prda_sys {
	...
	unsigned char	t_thrpreempt;	/* Thread nonpreemptive mutex held */
	unsigned char	t_syspreempt;	/* Sys nonpreemptive mutex held */
	unsigned char	t_yield;	/* pthread should sched_yield */
	unsigned char	t_unusedchar;
...
#define PRATTACH_PRIVATE  0x2		/* perform address space attach
					 * using private memory option */

#define	PRATTACH_MASK (PRATTACH_ALIGN|PRATTACH_PRIVATE)	/* allowable flags */
...
#define PR_THREAD_TIMEDBLOCK 6	/* block calling thread with timeout */
#define PR_THREAD_UNBLOCK_FLAGS 7 /* unblock named thread with extra flags */
#define PR_THREAD_BLOCKSET 8	/* set blocking counts for current uthread */

#define PR_THREAD_UNBLOCK_NOLATESLP 1 /* BROKEN: Formerly used by libpthread */
