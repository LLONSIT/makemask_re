...
#define JOB_NO_RESET	0x00000004
...
typedef struct cpu_set_s {
	...
	cnodemask_t	cs_mems;
	cpumask_t	cs_pad;
	cnodemask_t	cs_nodes;
	void		*cs_memctl;
	kthread_t 	*cs_rt_gq;
	lock_t		cs_gqlock;
	int		cs_cpu_advisory;
	int		cs_cpu_mandatory;
	uint64_t	cs_mem_advisory;
	uint64_t	cs_mem_mandatory;
} cpuset_t;
#pragma set type attribute cpu_set_s align=128
...
#define cpuset_flags(kt) cpusets[(kt)->k_cpuset].cs_flags
#define cpuset_nodemask(kt) *(cpusets[(kt)->k_cpuset].cs_mems)
#define cpuset_memory_ctl(kt) cpusets[(kt)->k_cpuset].cs_memctl

#define cpuset_open(index)	((cpusets[(index)].cs_flags & \
					CPUSET_CPU_EXCLUSIVE) == 0)
#define cpuset_isglobal(index)	((index) == CPUSET_GLOBAL) 

/* Check if a thread running in k_cpuset can run on _cpu */
#define cpuset_runok(k_cpuset, _cpu) ((cpuset_isglobal(k_cpuset) && \
				CPUMASK_TSTB(global_cpuset.cs_cpus,_cpu)) ||\
					(!cpuset_isglobal(k_cpuset) && \
					((k_cpuset) == cpu_to_cpuset[_cpu])))

/*
 * Below are private run queue prototypes
 */

/* These are run queue state values */
#define C_RESTRICTED	0x01	/* CPU is restricted */
#define C_SORTED	0x02	/* Run queue is sorted */
...
int		for_each_thread_init_batch(job_t *);
int		batch_initialize(kthread_t *, job_t *);
