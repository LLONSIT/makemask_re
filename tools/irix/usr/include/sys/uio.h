...
#define	IS_USERSPACE(f)	((f) == UIO_USERSPACE || (f) == UIO_USERISPACE)

#if CELL_CAPABLE
typedef enum uio_base { UIO_LOCAL, UIO_REMOTE } uio_base_t;
void set_local_uio_segflg(uio_t *, uio_seg_t, uio_base_t);
#endif /* CELL_CAPABLE */
...
void	uioupdate(struct uio *, ssize_t);
short	get_local_uio_segflg(uio_t *);
