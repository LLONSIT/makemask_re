...
struct vnodestats {
	...
	unsigned int	vn_insempty;	/* # times vn_insert called on empty vnode */
	unsigned int	vn_unlempty;	/* # times vn_unlink called on empty vnode */
	unsigned int	vn_pad[2];	/* allocated for future expansion */
};
...
#define CELLINFO	private.ksaptr->ci
