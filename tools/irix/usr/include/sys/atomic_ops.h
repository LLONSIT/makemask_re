...
static __inline uint atomicFieldAssignUint(uint *l, uint f, uint b) 
{
	uint tmp;
	do {
		tmp = *l;
...
static __inline long atomicFieldAssignLong(long *l,long f,long b) 
 {
	long tmp;
	do {
		tmp = *l;
...
extern int	test_and_set_int64(__int64_t *loc, __int64_t new);
...
#define atomicAddLongHot2(cp,val,pp)	atomicAddInt64Hot2(cp,val,pp)
#else /* !MP */
...
#define atomicAddLongHot2(cp,val,pp)	__add_and_fetch(cp,val)
...
#define atomicAddLongHot2(cp,val,pp)	atomicAddLong(cp, val)
