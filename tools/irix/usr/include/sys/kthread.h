...
#define KT_RESERVED		0x01000000 /* Reserved, don't use */
...
#define	KT_CB_INTERRUPT		0X10000000 /* thread callback intr by cxfs */
#define KT_SNAPSHOT		0x20000000 /* thread has snapshot locked */
#define KT_FAILSOFT_TRAP	0x40000000 /* failsoft trap due to panic */
...
struct kthread {
	...
	eframe_t	*k_eframe;	/* exception frame if interrupted */
        ...
        __int64_t       k_cprepspace[6]; /* space for cell stuff: bla ptr, svc
                                            ...
                                            and 1 32-bit field,
                                            just in case. */
#endif /* CELL_CAPABLE || CELL_PREPARE */
        /*
         * Memory affinity
         */
	...
	cnodeid_t	k_affnode;
#ifdef NUMA_BASE
	cnodeid_t	k_zonenode;	/* Default node for zone allocations */
#endif /* NUMA_BASE */
...
#define KT_ISLOCAL(kt) (KT_ISMR(kt) || KT_ISKB(kt) || \
				(KT_ISBOUND(kt) && \
					(is_bcritical(kt) || \
					(KT_ISUTHREAD(kt) && KT_ISGANG(kt)))))
...
#define thread_interrupted(kt)	\
	((kt)->k_flags & (KT_INTERRUPTED|KT_CB_INTERRUPT))
...
/* function prototypes */
extern int	thread_interrupt(struct kthread *, int *, int);
...
extern void xthread_table_init(void *, int);
