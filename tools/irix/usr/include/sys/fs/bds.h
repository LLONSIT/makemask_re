...
/* don't change this */
#define BDS_DEFAULT_TCP_PORT    2050
#define	BDS_DEFAULT_STP_PORT	1000
...
/* These #defines are here for older fcntl.h's */
...
#ifndef F_BDS_FSOPERATIONS
/* F_BDS_FSOPERATIONS uses 16 sequential fcntl cmd values as of IRIX 6.5 */
#define F_BDS_FSOPERATIONS	4096	/* get xfs real-time statistics */
/* 4096 ... 4111 are reserved for F_BDS_FSOPERATIONS */
#endif
...
#define BDSATTRF_BUFFERS_DOUBLE		0x40
...
#define BDS_DATAOPEN    'D'             /* Open second DATA channel ala ST */  
...
#define BDS_OPEN_BUF_DOUBLE 512	/* tell BDS to double buffer */
...
struct	_BDS_rdwr {		/* command field is 'r' or 'w' */
	uint64	offset;
	uint64	length;
#define BDS_RDWR_CTLCHANNEL   1 /* use the ctl channel for data mvmt*/ 
        uint64  flags;
};	/* 24 bytes */
...
struct  _BDS_dataopen {
        uint64  client_port;    /* port client is listening on */
};
...
typedef	struct {		/* BDS version 2.0 message */
	...
	union	{
		...
		struct	_BDS_ack	bds_ack;
	        struct  _BDS_dataopen   bds_dopen;
		uint64			bds_errno;
...
#define	bds_ack2	bds_u.bds_ack.val2
#define	bds_error	bds_u.bds_errno
#define bds_dport       bds_u.bds_dopen.client_port
