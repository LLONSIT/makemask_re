...
typedef struct vnode {
	...
	uchar_t		v_reserved1;		/* reserved for future use */
	uchar_t		v_reserved2;		/* reserved for future use */
...
#define VEXPORTING	0x40000 /* export in progress */
#define VWAITEXPRT	0x80000 /* threads waiting for export to complete */
...
#define	VREPLICABLE	0x800000 /* obsolete, never implemented */
#define	VNONREPLICABLE	0x1000000 /* obsolete, never implemented */
...
#define VRESERVED1	0x40000000 /* VTEARDOWN_SHAKE in cxfs */
...
#define VUMASK	0x0004		/* apply the users UMASK */
...
#define VNODE_TO_FIRST_BHV(vp)						     \
	((BHV_IS_BHVL(&(vp)->v_bh))? BHV_NEXT(BHV_HEAD_FIRST(&(vp)->v_bh)) : \
				     BHV_HEAD_FIRST(&(vp)->v_bh))
#define	VN_BHV_HEAD(vp)			((vn_bhv_head_t *)(&((vp)->v_bh)))
#define VN_BHV_READ_LOCK(bhp)		BHVL_READ_LOCK(bhp)
#define VN_BHV_READ_UNLOCK(bhp)		BHVL_READ_UNLOCK(bhp)
#define VN_BHV_WRITE_LOCK(bhp)		BHVL_WRITE_LOCK(bhp)
#define VN_BHV_WRITE_UNLOCK(bhp)	BHVL_WRITE_UNLOCK(bhp)
#define VN_BHV_WRITE_TO_READ(bhp)	BHVL_WRITE_TO_READ(bhp)
#define VN_BHV_IS_READ_LOCKED(bhp)	BHVL_IS_READ_LOCKED(bhp)
#define VN_BHV_NOT_READ_LOCKED(bhp)	BHVL_NOT_READ_LOCKED(bhp)
#define VN_BHV_IS_WRITE_LOCKED(bhp)	BHVL_IS_WRITE_LOCKED(bhp)
#define VN_BHV_LOCK_MINE(bhp)		BHVL_LOCK_MINE(bhp)
#define VN_BHV_NOT_WRITE_LOCKED(bhp)	BHVL_NOT_WRITE_LOCKED(bhp)
#define	VN_BHV_TRYACCESS(bhp)		BHVL_TRYACCESS(bhp)
#define	VN_BHV_TRYPROMOTE(bhp)		BHVL_TRYPROMOTE(bhp)
#define	VN_BHV_TRYUPDATE(bhp)		BHVL_TRYYPDATE(bhp)
#define	VN_BHV_AM_WRITE_OWNER(bhp)	BHVL_AM_WRITE_OWNER(bhp)
#define	VN_BHV_WRITE_TO_READ(bhp)	BHVL_WRITE_TO_READ(bhp)
...
#define vn_bhv_chain_insert(bhp,bdp)	bhv_chain_insert(bhp,bdp)
...
/*
 * Current vnodeops_t behavior version
 */
#define	VN_BHV_VERS_CUR		BHV_VERS_CUR

/*
 * VOP's.  
 */
#define	VN_BHV_VERSION_READ_LOCK(bhp)	BHV_VERSION_READ_LOCK((bhp))
#define	VN_BHV_VERSION_READ_UNLOCK(bhp)	BHV_VERSION_READ_UNLOCK((bhp))
#define	VN_BHV_VERSION_READ_LOCK_RV(bhp,rv)   BHV_VERSION_READ_LOCK_RV((bhp),rv)
#define	VN_BHV_VERSION_READ_UNLOCK_RV(bhp,rv) BHV_VERSION_READ_UNLOCK_RV((bhp),rv)
...
{									\
	int locked;							\
	ASSERT(&(vp) != vpp);						\
	VN_BHV_VERSION_READ_LOCK_RV(&(vp)->v_bh,locked);		\
	rv = _VOP_(vop_open, vp)((vp)->v_fbhv, vpp, mode, cr);		\
	VN_BHV_VERSION_READ_UNLOCK_RV(&(vp)->v_bh,locked);		\
}
#define	VOP_CLOSE(vp,f,c,cr,rv) 					\
{									\
	VN_BHV_VERSION_READ_LOCK(&(vp)->v_bh);				\
	rv = _VOP_(vop_close, vp)((vp)->v_fbhv,f,c,cr);			\
	VN_BHV_VERSION_READ_UNLOCK(&(vp)->v_bh);			\
}
...
{									\
	VN_BHV_VERSION_READ_LOCK(&(dvp)->v_bh);				\
	rv = _VOP_(vop_create, dvp)((dvp)->v_fbhv,p,vap,ex,mode,vpp,cr);\
	VN_BHV_VERSION_READ_UNLOCK(&(dvp)->v_bh);			\
}
...
{									\
	VN_BHV_VERSION_READ_LOCK(&(vp1)->v_bh);				\
	rv = _VOP_(vop_realvp, vp1)((vp1)->v_fbhv, vp2);		\
	VN_BHV_VERSION_READ_UNLOCK(&(vp1)->v_bh);			\
}
/*
...
{									\
	vnode_t *_myvp_ = vp;						\
	VN_BHV_VERSION_READ_LOCK(&(_myvp_)->v_bh);			\
	(void)_VOP_(vop_strategy, _myvp_)((_myvp_)->v_fbhv, bp);	\
	VN_BHV_VERSION_READ_UNLOCK(&(_myvp_)->v_bh);			\
}
...
/* 
 * It is possible that we will want to insert an interposer under
 * some very controlled conditions during a COVER.  During those
 * cases, the behavior lock could be released, and reacquired for
 * writing.  The code doing such must handle any change in the 
 * behavior list.
 */
#define VOP_COVER(cvp, uap, attrs, cr, rv) 				\
{       								\
	VN_BHV_VERSION_READ_LOCK(&(cvp)->v_bh);       			\
        rv = _VOP_(vop_cover, cvp)((cvp)->v_fbhv, uap, attrs, cr); 	\
	VN_BHV_VERSION_READ_UNLOCK(&(cvp)->v_bh);       		\
}
 ...
 *
 * Note: vp is put on the stack because the unlock needs to get to the
 *       behavior, VOP_COMMIT is mostly called with bp->b_vp as the
 *       argument for vp, this is a problem because the nfs V3 commit
 *       can call chunkunhash() which can NULL out the bp->b_vp, however
 *       the vnode is still around and usable but the buffer doesn't
 *       point to it when we get back from the _VOP_() call, therefore,
 *       we put the vnode pointer on the stack for use by the unlock
 */
#define VOP_COMMIT(vp,bp,rv)						\
{									\
        vnode_t *vp_ptr = vp;                                           \
        VN_BHV_VERSION_READ_LOCK(&(vp_ptr)->v_bh);                      \
        rv = _VOP_(vop_commit, vp_ptr)((vp_ptr)->v_fbhv, bp);           \
        VN_BHV_VERSION_READ_UNLOCK(&(vp_ptr)->v_bh);                    \
}
...
/*
 * VOP versions called when the behavior lock has been explicitly taken
 */
#define BHVLOCKED_VOP_GETATTR(vp, vap, f, cr, rv)		  	\
{									\
	bhv_desc_t      *bdp;						\
	if (BHV_IS_BHVL(VN_BHV_HEAD(vp))) { 				\
		bdp = BHV_NEXT((vp)->v_fbhv);				\
	} else							       	\
		bdp = (vp)->v_fbhv;					\
	rv = (*((vnodeops_t *)(bdp)->bd_ops)->vop_getattr)(bdp, vap, f, cr);\
}

#define BHVLOCKED_VOP_SETATTR(vp, vap, f, cr, rv)		  	\
{									\
	bhv_desc_t      *bdp;						\
	if (BHV_IS_BHVL(VN_BHV_HEAD(vp))) { 				\
		bdp = BHV_NEXT((vp)->v_fbhv);				\
...
/*
 * Behavior lock is set above all possible others to insure it is always on top
 */
#define	VNODE_POSITION_BHVLOCK	 (BHV_POSITION_TOP+1)	/* must be at top */
#define	VNODE_POSITION_OPLOCK	 (VNODE_POSITION_BASE+10)  /* oplock hooks      */
#define	VNODE_POSITION_IMON	 (VNODE_POSITION_BASE+9)  /* inode monitor     */
#define	VNODE_POSITION_REPL	 (VNODE_POSITION_BASE+8)  /* obsolete */
#define	VNODE_POSITION_SPECFS_DC (VNODE_POSITION_BASE+7)  /* specfs DC layer   */
#define	VNODE_POSITION_DMAPI_BNC (VNODE_POSITION_BASE+6)  /* dmapi bounce layer   */
#define	VNODE_POSITION_CFS_DC	 (VNODE_POSITION_BASE+5)  /* CFS client	       */
#define	VNODE_POSITION_CFS_DS	 (VNODE_POSITION_BASE+4)  /* CFS server	       */
#define	VNODE_POSITION_SPECFS_DS (VNODE_POSITION_BASE+3)  /* specfs DS layer   */
#define	VNODE_POSITION_SPECFS_AT (VNODE_POSITION_BASE+2)  /* specfs AT layer   */
#define VNODE_POSITION_ENDCAP	 (VNODE_POSITION_BASE+1)  /* inactive-teardown handler */
...
extern void	vn_expunge(struct vnode *);
extern void	vn_purge(struct vnode *, vmap_t *);
extern vnode_t *vn_get(struct vnode *, vmap_t *, uint);
extern void	vn_bitlock_wait(vnode_t *, int , unsigned int *, unsigned int , int);
extern void	vn_broadcast(vnode_t *);
extern uint64_t vn_get_generation(void);
...
/*
 * Obsolete Macros for replication which was never implemented.
 */
...
#define	VN_CLRNONREPLICABLE(vp)	 /* in vnode.c */

#define	VN_ISREPLICABLE(vp)		(0) /* many spots */
...
#define ATTR_NOTOK	0x800	/* Don't get any tokens	    */
