...
#if !defined(SN0XXL) && !defined(SN1)	/* MAXCPUS 128p (64 nodes) or less */
...
#define CNODEMASK_ATOMCLR_BIT(p, bit) \
...
#define CNODEMASK_ATOMSET_MASK(__old, p, q)  { \
           (__old)._bits[0] =	 \
	      atomicSetUlong((unsigned long *)&(p)._bits[0], (q)._bits[0]);  \
           (__old)._bits[1] =    \
              atomicSetUlong((unsigned long *)&(p)._bits[1], (q)._bits[1]);  \
}
#define CNODEMASK_ATOMCLR_MASK(__old, p, q) {					\
           (__old)._bits[0] =				\
	      atomicClearUlong((unsigned long *)&(p)._bits[0], (q)._bits[0]); \
           (__old)._bits[1] =                           \
              atomicClearUlong((unsigned long *)&(p)._bits[1], (q)._bits[1]); \
}
