...
#ifdef DEBUG_BUFTAGGING

#define BT_RING_SIZE	8

typedef struct bt_location {
	short	linenum;
	short	filenum;
} bt_location_t;
#endif
...
typedef struct	buf {
	...
	ushort	b_aflags;		/* additional flags.  see below */
...
#endif	
#ifdef DEBUG_BUFTAGGING
	bt_location_t	bt_loc[BT_RING_SIZE];	/* ring buffer */
        lock_t  bt_lock;        	/* ring buffer spinlock */
	short	bt_head;		/* head of ring buffer */
#endif	
} buf_t;
...
#define B_XLVSPEC  0x0000000200000000LL /* Flag used by XLV to indicate that B_PAGEIO was set.
					 * Used by macsisort to place request on appropriate queue. */
...
#define B_XVM_PRIV 0x0000080000000000LL	/* buffer alloc out of xvm priv zone */
...
#define B_IO_ACCT \
		   0x0004000000000000LL /* A write _I/O_ is currently in
		   			   progress and accounted for. Used
					   by the throttle */
#define B_DIRTY_ACCT \
		   0x0008000000000000LL /* A buffer that has been dirtied
		   			   but not yet written out. Used
					   by the throttle */
#define B_NFS_JUKEBOX \
		   0x0010000000000000LL /* NFS Jukebox error occurred on the
		   			   buffer - debug trapping only */
#define B_NFS_JUKEBOX_STALE \
		   0x0020000000000000LL /* NFS Jukebox error occurred on a
		   			   B_STALE buffer. B_STALE is removed,
					   this flag set and when the jukebox
					   error clears B_STALE is restored. */

#define BP_ISMAPPED(bp)	((bp->b_flags & (B_PAGEIO|B_MAPPED)) != B_PAGEIO)
#define BUF_IS_IOSPL(bp) (bp->b_flags & (B_GR_BUF|B_PRV_BUF))

/*
 *	These flags are kept in b_aflags
 */
#define BA_PINWAITER	0x01	/* Someone is waiting for unpin */
#define	BA_DONTRECLAIM	0x02	/* Do not reclaim buffer */
#define	BA_GLOBAL_BUF	0x04	/* Global buffer table buffer */

#define	B_GET_AFLAG(bp, flag)	(bp)->b_aflags & flag
#define	B_SET_AFLAG(bp, flag)	atomicSetUint((uint*)&(bp)->b_pincount, (flag))
#define	B_CLEAR_AFLAG(bp, flag)	atomicClearUint((uint*)&(bp)->b_pincount, (flag))
...
typedef struct bwait_pin {
	...
	lock_t	bwp_lock;	/* lock for this pin structure */
} bwait_pin_t;	

/* Cacheline separate the entries in the bwait_pin array */
#ifdef MP
#pragma set type attribute bwait_pin_t align=128
#endif
...
/*
 * Parameter/reporting structure for binval_dev/bflush_dev
 */
typedef struct bfidev {
	dev_t		bfid_dev;	/* Device we are handling. */
	int             bfid_flags;     /* Control flags. */
        struct {			/* Results returned. */
        	int     bfidr_wrote;	/* Buffers we wrote. */
	        int     bfidr_vskip;	/* Buffers skipped because there was 
					   vnode associated. */
        	int     bfidr_lskip;	/* Buffers skipped becase we could
					   no lock them. */
                int     bfidr_mchange;  /* Buffers skipped because they did
					   not match the criteria after we
					   locked them. */
                int     bfidr_pskip;    /* Buffers skipped because they were
					   pinned. */
	        int     bfidr_mkstale;	/* Buffers we made stale. */
	        int     bfidr_stale;	/* Buffers we left stale. */
	} bfid_r;
} bfidev_t;

#define BFID_TRYLOCK	0x01		/* When getting the buffer lock, do
					   not wait. */
#define BFID_NOVNBUF    0x02		/* Skip buffers associated with */
                                        /* vnodes. */
#define BFID_NOIOWAIT   0x04            /* Applies to bflush_dev only: Do */
                                        /* not wait for writes to complete. */
                                        /* When BFID_TRYLOCK is passed to */
                                        /* bflush_dev, BFID_NOWAIT is highly */
                                        /* recommended since the wait for IO */
                                        /* locks buffers unconditionally. */
...
/* 
 * NOTE WELL!!! This list *must* be kept up to date. If you need to
 * add tagging to a new file, then add to the end of this list
 */

#define BT_FS_BIO		0	/* sgi/fs_bio.c */
#define BT_CHUNKIO		1	/* sgi/chunkio.c */
#define BT_PLEX_CPY		2	/* io/xlv/plex_cpy.c */
#define BT_XLV			3	/* io/xlv/xlv.c */
#define BT_GRIO_SUB		4	/* io/grio/grio_sub.c */
#define BT_BPQUEUE		5	/* io/bpqueue.c */
#define BT_DISKSUB		6	/* io/disksub.c */
#define BT_DKSC			7	/* io/dksc.c */
#define BT_XFS_MOUNT		8	/* fs/xfs/xfs_mount.c */
#define BT_XFS_RW		9	/* fs/xfs/xfs_rw.c */
#define BT_XFS_BUF_ITEM		10	/* fs/xfs/xfs_buf_item.c */
#define BT_CXFS_CLIENT_MOUNT	11	/* fs/cxfs/xfs/client/xfs_mount.c */
#define BT_CXFS_CLIENT_RW	12	/* fs/cxfs/xfs/client/xfs_rw.c */
#define BT_NFS_VNODEOPS		13	/* fs/nfs/nfs_vnodeops.c */

void buftag1_func(buf_t *a, short b, short c);
void buftag3_func(buf_t *a, short b, short c);

#define buftag1(a, b, c) buftag1_func(a, b, c)
#define buftag3(a, b, c) buftag3_func(a, b, c)

#else

#define buftag1(a, b, c)
#define buftag3(a, b, c)
...
extern buf_t	*getrbuf_zone(int, zone_t *);
...
extern void     binval_dev(bfidev_t *);
extern void     bflush_dev(bfidev_t *);
...
extern int	buf_avoid_add(dev_t);
extern int	buf_avoid_delete(dev_t);
extern int	buf_avoid_search(dev_t);
extern void	clrbuf(buf_t *);
extern void	freerbuf(buf_t *);
extern void	freerbuf_zone(buf_t *, zone_t *);
