...
#ident  "$Id: tpucom.h,v 1.7 2002/05/24 19:45:34 letsche Exp $"
...
#define TPU_LOAD_CACHE	(TPU_MAGIC |  5)
#define TPU_WALK_CACHE	(TPU_MAGIC |  7)
...
typedef struct tpud_run_s {
	...
	__uint64_t		_reserved_0[2];			/* reserved - must be zero */
	...
	__uint64_t		_reserved_1[11];		/* reserved - must be zero */
	__uint64_t		timeout;			/* timeout in seconds */
	__uint64_t		ldi_offset;			/* byte offset to start of LDI coef */
	__uint32_t		f_pinned	: 1,		/* memory has been pinned by application */
				f_timeout_in_ms : 1,		/* timeout value is expressed in milliseconds, not seconds */
				f_addrs_cached  : 1,		/* virtual addresses in page_addr have been cached with ioctl(TPU_LOAD_CACHE) */
				f_reserved	: 29;		/* reserved - must be zero */
	__uint32_t		page_size;			/* page size in bytes */
...
/*
 * User-level physical->virtual address structure.
 */
typedef struct tpud_mem_s {
	__uint32_t  reserved;
	__uint32_t  page_size;
	__uint64_t  page_count;
