...
#ident  "$Id: tpudrv.h,v 1.5 2002/05/24 19:45:34 letsche Exp $"
 ...
 */
typedef struct tpu_physaddr_cache_entry {
	pid_t   pid;				/* process id */
	size_t  len;				/* length of the page being cached */
	caddr_t uaddr;				/* virtual address */
	caddr_t paddr;				/* physical address */
	__int32_t cookie;			/* userdma cookie */
	struct tpu_physaddr_cache_entry *next;
} tpu_physaddr_cache_entry_t;


typedef struct tpu_physaddr_hash {
	tpu_physaddr_cache_entry_t *physaddr_list;
} tpu_physaddr_hash_t;

extern const int tpu_physaddr_cache_size;
#define TPU_PHYSADDR_UHASH(pid, uaddr) \
	((((ulong)(pid) ^ ((ulong)(uaddr) >> 20))) % tpu_physaddr_cache_size)
...
typedef struct tpu_soft_s {
	...
	tpu_physaddr_hash_t	*tpu_physaddr_cache;	/* physical->virtual address cache */
