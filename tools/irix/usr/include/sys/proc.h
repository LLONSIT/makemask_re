...
/* 
 * This structure replaces existing proc_proxy_t members so proc_proxy_t 
 * stays the same size, but still allows additional data required to fix
 * problems with coredumping multi-thread processes (PV 940292).
 */
typedef struct coredump_war_s {
	struct vnode	*coredump_vp;		/* corefile vp */
	off_t           coredump_fileoff;  	/* next thread's data offset 
		                                * in file */
	int		coredump_todo;		/* Number of threads to be 
					         * processed by coredump. */
	int		coredump_nthreads;	/* next thread's index */
						/* in corefile */
} coredump_war_t;
...
#define prxy_spsize	prxy_sigstack.ss_size
	...
	coredump_war_t  *prxy_coredump_war;	/* ptr to add'l coredump 
						 * data to WAR impacting
						 * other kernel modules. */
#define prxy_coredump_vp	prxy_coredump_war->coredump_vp
#define prxy_coredump_fileoff	prxy_coredump_war->coredump_fileoff
#define prxy_coredump_todo	prxy_coredump_war->coredump_todo
#define prxy_coredump_nthreads	prxy_coredump_war->coredump_nthreads
	mutex_t		prxy_coredump_mutex;	/* held while thread */
						/* is dumping */
	sema_t		prxy_coredump_wait;	/* Synch queue to hold
						 * threads in processes until
						 * all threads are waiting to 
						 * dump and after all have 
						 * completed dumping.
						 */
...
#define prxy_to_thread(prxy)	(ASSERT((!(IS_THREADED((prxy)))) || \
				...
				(prxy)->prxy_threads)
#define uscan_forloop(prxy, ut) for (ASSERT(uscan_held(prxy)), \
                                     (ut) = (prxy)->prxy_threads; \
                                     (ut) != NULL; \
                                     (ut) = (ut)->ut_next)
...
/*
 * Used for frs_intr_thread_sendsig().  The first two members map
 * to frs_threadid_t and indicate to whom we're sending the signal.
 */
typedef struct _async_frs_sig {
	pid_t	s_pid;
	uthreadid_t	s_tid;
	int	s_sig;
} _async_frs_sig_t;
...
typedef struct async_vec_s {
	...
	union {
		...
		_async_frs_sig_t	async_frs;	/* frs_intr_thread_sendsig */
	} async_arg;
} async_vec_t;
