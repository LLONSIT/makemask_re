...
extern void cpuset_mem_init(void);
...
extern int memaff_setaff(kthread_t*, cnodeid_t);
...
/* this interface is no longer used */
extern int migr_check_migratable(pfd_t *);

extern int migr_pfdat_xfer(pfd_t* old_pfd, pfd_t* new_pfd);
extern int migr_page_migrate(pfd_t* old_pfd, pfd_t* new_pfd);
extern int migr_engine(pfn_t old_pfn, cnodeid_t dest_node, int caller);
extern void migr_pagemove(pfd_t* old_pfd, pfd_t* new_pfd, int migr_vehicle);
