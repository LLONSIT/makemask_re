 ...
 *
 * NOTE: PIC WAR. PV#854697.  
 * PIC does not allow writes just to [31:0] of a 64-bit register.  When
 * writing PIC registers, always write the entire 64 bits.
 */
...
/*
 * Bridge error interrupts start/end bits.  In bridge.h because stand/arcs
 * needs access to them too (they use to be in pcibr_private.h).
 */
...
#define BRIDGE_ISR_MULTI_ERR		(0x1ull << 31)	/* bridge only */
#define BRIDGE_ISR_PMU_ESIZE_FAULT	(0x1ull << 30)	/* bridge only */
#define BRIDGE_ISR_PAGE_FAULT		(0x1ull << 30)	/* xbridge only */
#define BRIDGE_ISR_UNEXP_RESP		(0x1ull << 29)
#define BRIDGE_ISR_BAD_XRESP_PKT	(0x1ull << 28)
#define BRIDGE_ISR_BAD_XREQ_PKT		(0x1ull << 27)
#define BRIDGE_ISR_RESP_XTLK_ERR	(0x1ull << 26)
#define BRIDGE_ISR_REQ_XTLK_ERR		(0x1ull << 25)
#define BRIDGE_ISR_INVLD_ADDR		(0x1ull << 24)
#define BRIDGE_ISR_UNSUPPORTED_XOP	(0x1ull << 23)
#define BRIDGE_ISR_XREQ_FIFO_OFLOW	(0x1ull << 22)
#define BRIDGE_ISR_LLP_REC_SNERR	(0x1ull << 21)
#define BRIDGE_ISR_LLP_REC_CBERR	(0x1ull << 20)
#define BRIDGE_ISR_LLP_RCTY		(0x1ull << 19)
#define BRIDGE_ISR_LLP_TX_RETRY		(0x1ull << 18)
#define BRIDGE_ISR_LLP_TCTY		(0x1ull << 17)
#define BRIDGE_ISR_SSRAM_PERR		(0x1ull << 16)
#define BRIDGE_ISR_PCI_ABORT		(0x1ull << 15)
#define BRIDGE_ISR_PCI_PARITY		(0x1ull << 14)
#define BRIDGE_ISR_PCI_SERR		(0x1ull << 13)
#define BRIDGE_ISR_PCI_PERR		(0x1ull << 12)
#define BRIDGE_ISR_PCI_MST_TIMEOUT	(0x1ull << 11)
#define BRIDGE_ISR_GIO_MST_TIMEOUT	BRIDGE_ISR_PCI_MST_TIMEOUT
#define BRIDGE_ISR_PCI_RETRY_CNT	(0x1ull << 10)
#define BRIDGE_ISR_XREAD_REQ_TIMEOUT	(0x1ull << 9)
#define BRIDGE_ISR_GIO_B_ENBL_ERR	(0x1ull << 8)
#define BRIDGE_ISR_INT_MSK		(0xffull << 0)
#define BRIDGE_ISR_INT(x)		(0x1ull << (x))
...
#define PIC_IMR_PCIX_REQ_TOUT		PIC_ISR_PCIX_REQ_TOUT
...
#define BRIDGE_IRR_MULTI_CLR		(0x1ull << 6)
#define BRIDGE_IRR_CRP_GRP_CLR		(0x1ull << 5)
#define BRIDGE_IRR_RESP_BUF_GRP_CLR	(0x1ull << 4)
#define BRIDGE_IRR_REQ_DSP_GRP_CLR	(0x1ull << 3)
#define BRIDGE_IRR_LLP_GRP_CLR		(0x1ull << 2)
#define BRIDGE_IRR_SSRAM_GRP_CLR	(0x1ull << 1)
#define BRIDGE_IRR_PCI_GRP_CLR		(0x1ull << 0)
#define BRIDGE_IRR_GIO_GRP_CLR		(0x1ull << 0)
 ...
 * 
 * Note that these are for Bridge/Xbridge.  PIC code should use the
 * PICBRIDGE_xxx macros defined below.
 */
...
#ifdef _STANDALONE
/* XXX These are only used in the PROM, kernel versions of these macros 
 * are defined in pcibr_private.h which isnt available in the PROM bld.
 * The kernel versions are also different (they changed when we did the
 * kernel cleanup (PV 895768)).  Until we cleanup the PROM, these will
 * have to stay
 */
#define is_xbridge(bridge) IS_XBRIDGE(bridge->b_wid_id)
#define is_pic(bridge) IS_PIC_BRIDGE(bridge->b_wid_id)

#define is_pcix(bridge) is_pic(bridge) ? \
			(bridge->p_wid_stat_64 & PIC_STAT_PCIX_ACTIVE) : 0
...
#ifdef _STANDALONE
extern int		alloc_bridge_rrb(void *bus_base, int bus_slot, int v, int ct);
extern int		free_bridge_rrb(void *bus_base, int bus_slot, int v, int ct);
extern void		init_bridge(widget_cfg_t *);
extern void		set_bridge_vector(void *bus_base, int bus_slot, int vec);
extern void		disable_bridge_intr(void *bus_base, int bus_slot);

extern void		bridge_clearnofault(void);
#endif
