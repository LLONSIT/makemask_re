...
extern int	reclock(struct vnode *, struct flock *, int, int, off_t,
				struct cred *);
/* reclock2 is for XFS/CXFS use only */
extern int	reclock2(struct vnode *, struct flock *, int, int, off_t,
				struct cred *, int, int, int *);
...
/* The following three routines are for XFS/CXFS use only */
extern int	cancel_lock_for_cxfs(struct vnode *, struct flock *, int);
extern int	cleanlocks_recovery(struct vnode *, pid_t, sysid_t);
extern void	convert_lock_for_cxfs(struct vnode *, struct vnode *);

extern flid_t	*sys_flid;

extern void cancel_blocked_locks(__uint32_t);
extern void cancel_blocked_locks_local(__uint32_t);
#ifdef DEBUG
