...
#define MTSEEK	21	/* Seek to the given block number.  Not all drives
	...
	* see MTSETAUDIO. */
#define MTRSV	22	/* Issue a unit reserve command */
#define MTRLS	23	/* Issue a unit release command */
#define	MTPREEMPT 24	/* Preempt active persistent reservation */
#define	MTCLEAR 25	/* Clear persistent reservation and keys */
...
/* 
 * Structure for MTSETVID.
 * Set Volume id in personality. This should be used by an application
 * which knows the volume id of the mounted tape. The volume id will be
 * printed in error messages and error reports.
 * Specify a NULL terminated string for the volume id.
 */
#define	MT_MAX_VID	6		/* Max char in vid, excluding the
					 * terminating 0. */
struct	mtvid {
	char	vid[MT_MAX_VID + 1];	/* volume id - NULL terminated string */
};
...
#define	MTSETVID	MTIOCODE('y')	/* Set VolumeID in personality daemon */
#define	MTPRSV		MTIOCODE('z')	/* Get persistent rsv out key */
...
#define	MTR_PREEMPT	28		/* Preempt persistent reservation */
#define	MTR_CLEAR	29		/* Clear persistent reservation and
					 * registered keys */
