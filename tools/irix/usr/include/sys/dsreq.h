 ...
 */
#ident "sys/dsreq.h: $Revision: 1.26 $"
...
#define I_MAX SCSI_MAXTARG	/* I_MAX and SCSI_MAXTARG are deprecated */
#define L_MAX SCSI_MAXLUN	/* L_MAX is deprecated                */
...
typedef  struct  dsconf  {
  ...
  u_char  dsc_bus;		/* number of this scsi bus           */
  u_char  dsc_imax;		/* max number ID-s per bus, any ctlr */
  u_char  dsc_lmax;		/* max number LUN-s per ID, any ctlr */
...
#define B8(s) B((s),56), B((s),48), B((s),40), B((s),32), \
	      B((s),24), B((s),16), B((s),8),   B1(s)
