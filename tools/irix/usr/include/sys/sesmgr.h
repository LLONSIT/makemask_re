...
int sesmgr_istsix(struct socket *, struct mbuf *, struct in_addr *, int *);

#define _SESMGR_CONFIGNOTE()	   ((sesmgr_enabled)? \
					sesmgr_confignote(): (void)0 )

/*
 * Session Manager Socket Layer Hooks
 */
int sesmgr_connect(struct socket *, struct mbuf *);
int sesmgr_accept(struct socket *, struct mbuf *);
int sesmgr_send(struct socket *, struct mbuf *);
int sesmgr_recv(struct socket *, struct mbuf *);

#define _SESMGR_CONNECT(a,b)	((sesmgr_enabled) ? \
					sesmgr_connect((a), (b)) : 0)
#define _SESMGR_ACCEPT(a,b)	((sesmgr_enabled) ? \
					sesmgr_accept((a), (b)) : 0)
#define _SESMGR_SEND(a,b)	((sesmgr_enabled) ? \
					sesmgr_send((a), (b)) : 0)
#define _SESMGR_RECV(a,b)	((sesmgr_enabled) ? \
					sesmgr_recv((a), (b)) : 0)
/*
 ...
 */
int ip_mac_inrange(struct ipsec *, struct t6rhdb_kern_buf *t6p);
int sesmgr_ip_clearance(struct ipsec *, struct ipsec *);
...
int  sesmgr_samp_kudp_send(struct socket *, struct mbuf **, struct in_addr);
int  sesmgr_samp_kudp_recv(struct socket *, struct mbuf **, struct in_addr);
int  sesmgr_samp_nfs_send(struct mbuf *, struct mbuf **, struct in_addr);
int  sesmgr_samp_nfs_recv(struct mbuf *, struct mbuf **, struct in_addr);
