...
#define MP_CPUSET	       35   /* cpuset related sysmp services */
...
#define MP_NUM_CPU_PER_NODE	49	/* Max number of CPUs per node */

/* Unavailable cpus */
#define MP_EXCLUDE		50	/* Exclude the processor from all work */
#define MP_UNEXCLUDE		51	/* Un-exclude the processor */

/* Control real-time clock interrupts */
#define MP_WARDRTC		52	/* Ward off RTC interrupts 
					 * from the processor */
#define MP_PERMITRTC		53	/* Permit RTC interrupts on
					 * the processor */

#define MP_SN1MACHTYPE		54	/* The type of SN1 based host */
...
#define MPSA_NODE_INFO64 34	/* Per node info -- larger values*/
/* Added in 6.5.17 */
#define MPSA_RMINFO64	35	/* 64 bit version of MPSA_RMINFO */
#define MPSA_MINFO64	36	/* real memory info - larger version */
#define MPSA_BUFTHROTTLE 37	/* buffer cache throttle stats */
#define MPSA_TCPIPSTATS2 38	/* extended TCP/IP stats (superset of
					MPSA_TCPIPSTATS) */
#define	MPSA_MAX	38	/* Max entries, >>End-of-Table<< */
...
/* This is to solve an overflow problem when using a 64 bit kernel
 * and using 32 bit variables for 64 bit math.
 */
struct rminfo64 {
        __uint64_t      freemem;        /* current pages free memory */
        __uint64_t      availsmem;      /* available real and swap memory */
        __uint64_t      availrmem;      /* available real memory */
        __uint64_t      bufmem;         /* attached to metadata cache */
        __uint64_t      physmem;        /* total real memory */
        __uint64_t      dchunkpages;    /* delwri pages */
        __uint64_t      pmapmem;        /* pages in pmap */
        __uint64_t      strmem;         /* pages of streams resources */
        __uint64_t      chunkpages;     /* chunk pages */
        __uint64_t      dpages;         /* dirty, unattached pages */
        __uint64_t      emptymem;       /* free pages not caching data */
        __uint64_t      ravailrmem;     /* unlocked real memory */
};
...
#ifdef _KERNEL
struct kthread;
extern int mp_mustrun(pid_t, cpuid_t, int, int);
extern void mp_runthread_anywhere(struct kthread *);
extern int mp_runanywhere(pid_t, int, int);
extern int mp_isolate(cpuid_t);
extern int mp_unisolate(cpuid_t);
#ifdef TILES_TO_LPAGES
