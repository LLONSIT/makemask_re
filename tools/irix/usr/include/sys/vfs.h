...
#define	VFS_BOPS(bdp)	\
        ((vfsops_t *)(bdp->bd_ops))		/* ops for behavior */
...
#define VFS_EILIMIT     64             /* maximum size for exported info */
...
#define VFS_CELLULAR	0x0800		/* fs is cxfs exported */
...
#define VFS_CELLROOT	0x8000		/* not used, do not use - only left
					 * here to avoid build problems with
					 * third party code, VFS_QUIESCED
					 * has the same value, but will not be
					 * checked on anything but cxfs */
#define VFS_QUIESCED	0x8000		/* filesystem will block vop calls */
#define VFS_DOXATTR	0x10000		/* tell server to trust us with
					   attributes */
#define VFS_DEFXATTR	0x20000		/* use default values for
					   system attributes */
#define VFS_DMIMNT	0x40000		/* hook up dmapi bhv layers */

/* 
 * the scope of these bits is within a cell, when adding new bits to
 * vfs_flag one should also add those bits in VFS_CELL_BITS
 */
#define VFS_CELL_BITS \
	(VFS_REMOUNT|VFS_MLOCK|VFS_MWAIT|VFS_MWANT|VFS_OFFLINE|VFS_QUIESCED| \
	 VFS_DMIMNT)
...
#define UNMOUNT_NODEVCLOSE 0x04         /* do not close devices, i.e. ddevp */
...
typedef struct vfsops {
	...
	void	(*vfs_init_vnode)(bhv_desc_t *,
				  struct vnode *,
				  int);
} vfsops_t;

/*
 * Current vfsops_t behavior version
 */
#define	VFS_BHV_VERS_CUR		BHV_VERS_CUR

#define	VFS_BHV_VERSION_READ_LOCK(bhp)	 BHV_VERSION_READ_LOCK((bhp))
#define	VFS_BHV_VERSION_READ_UNLOCK(bhp) BHV_VERSION_READ_UNLOCK((bhp))
#define	VFS_BHV_VERSION_READ_LOCK_RV(bhp,rv)   BHV_VERSION_READ_LOCK_RV((bhp),(rv))
#define	VFS_BHV_VERSION_READ_UNLOCK_RV(bhp,rv) BHV_VERSION_READ_UNLOCK_RV((bhp),(rv))

#define VFS_NEXT_VERSION(bdp, vers)	\
{	\
	while (bdp && VFS_BOPS(bdp)->vfs_ops_version < VFS_OPSVER_INITVN) { \
		bdp = bdp->bd_next; \
	} \
}
...
{	\
	VFS_BHV_VERSION_READ_LOCK(&(vfsp)->vfs_bh); \
	rv = (*(VFS_FOPS(vfsp)->vfs_mntupdate))((vfsp)->vfs_fbhv, mvp, uap, cr);    \
	VFS_BHV_VERSION_READ_UNLOCK(&(vfsp)->vfs_bh); \
}
#define VFS_DOUNMOUNT(vfsp,f,vp,cr, rv)	\
{	\
	int locked = 0;	\
	VFS_BHV_VERSION_READ_LOCK_RV(&(vfsp)->vfs_bh, locked); \
	rv = (*(VFS_FOPS(vfsp)->vfs_dounmount))((vfsp)->vfs_fbhv, f, vp, cr); \
	VFS_BHV_VERSION_READ_UNLOCK_RV(&(vfsp)->vfs_bh, locked); \
}
...
#define	VFS_INIT_VNODE(vfsp, vp, arg) \
{									   \
	bhv_desc_t *_bdp = (vfsp)->vfs_fbhv;				   \
	VFS_NEXT_VERSION(_bdp, VFS_OPSVER_INITVN);			   \
	if (_bdp)							   \
		(*(VFS_BOPS(_bdp)->vfs_init_vnode))(_bdp, vp, arg);	   \
}
...
#define VFS_OPSVER_65MR		0	/* This is where we presume third- */
                                        /* party fs's are. */
#define VFS_OPSVER_GROVE        1       /* Extensions that all of the fs's */
                                        /* in the kernel source tree have */
                                        /* for grove. */
#define	VFS_OPSVER_INITVN	2	/* Extension that includes init */
					/* vnode callout */

#define VFS_OPSFL_DEAD_SERVER 0x01      /* Server has died, need umount/mount */
#define VFS_OPSFL_FRLOCK2     0x02      /* CXFS- use extended frlock calls
					   (VOP_FRLOCK2, VOP_RWLOCKING)
					   instead of VOP_FRLOCK, VOP_RWLOCK  */
 ...
 */
#define	VFS_POSITION_BHVLOCK	VFS_POSITION_TOP+1	/* bhv lock layer */
#define VFS_POSITION_DMAPI_BNC	VFS_POSITION_TOP-10	/* dmapi_bnc layer */
#define	VFS_POSITION_CFS_DS	VFS_POSITION_TOP-20  	/* CFS server */
...
extern int	vfs_trybusy(struct vfs *);  /* no wait mark busy */
