...
#define	PIOCCKPTGETCSA (PIOC|176)	/* get CSA accounting info */
#define	PIOCCKPTSETCSA (PIOC|177)	/* set CSA accouting info */
#define	PIOCCKPTGETJINFO (PIOC|178)	/* get job info */
#define	PIOCCKPTSETJINFO (PIOC|179)	/* set job info */
...
typedef struct ckpt_statbuf {
	...
	char		ckpt_fstype[_ST_FSTYPSZ];
        uid_t           ckpt_uid;       /* user ID of file's owner */
        gid_t           ckpt_gid;       /* group ID of the file's group */
...
/*
 * PIOCCKPTGETJINFO and PIOCCKPTSETJINFO
 */
typedef struct ckpt_jobinfo_arg {
	jid_t		jid;
	int		flags;
	int		refcnt;
	time_t		start;
	uid_t		uid;
	jres_t		data[JLIM_NLIMITS];
} ckpt_ji_arg_t;

/*
 * PIOCCKPTGETCSA and PIOCCKPTSETCSA
 */
typedef struct ckpt_csa_arg {
	accum_t		csa_vmem1;
	accum_t		csa_vmem2;
	accum_t		csa_vmem3;
	accum_t		csa_mem2;
