...
union scsi_read_capacity {
	struct {
		uint32_t	last_lba;
		uint32_t	last_lba_size;
	} cap10;
	struct {
		uint64_t	last_lba;
		uint32_t	last_lba_size;
...
struct	dksoftc {
	...
	union scsi_read_capacity *dk_drivecap;	/* Read Capacity parameter data */
	uint	 dk_blksz;	/* logical (from drive) blocksize of device */
	uint64_t dk_readflag;	/* flag to pass to physiock for reads */
	uint64_t dk_writeflag;	/* flag to pass to physiock for reads */
	uint64_t dk_bytecap;	/* drive capacity in bytes */
	uint64_t dk_blockcap;	/* drive capacity in blocks */
	...
	ushort	 dk_flags;
	char	 dk_2tblun;	/* If set, indicates lun is 2+ TB in size */
#if DKSC_SPINLOCKS
	lock_t	 dk_lock;
	int	 dk_lockspl;
#else
	...
	u_char	 dk_laststat;	/* last SC_* returned for lun */
	...
	struct diskqueue dk_queue; /* wait queue */
	...
	buf_t	*rbuf_chain;	/* chain of free raw buffers */
#if DKSC_SPINLOCKS
	lock_t	 rbuf_chain_lock;
#else
	mutex_t	 rbuf_chain_lock;
#endif
	int	 rbuf_count;	/* number of raw buffers used */
};

#define DK_INQTYP_SIZE 4	/* used for dksoftc.dk_inqtype */
#define DK_DRIVECAP_SIZE sizeof(union scsi_read_capacity)
