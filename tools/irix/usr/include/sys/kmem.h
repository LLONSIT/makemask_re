 ...
 *	kmem_zone_alloc_node_hint(zone, node, flags)
 *		-- alloc a zone unit from the zone;
 *	kmem_zone_zalloc_node_hint(zone, node, flags)
 * 		-- alloc a zeroed zone unit from the zone;
 ...
 *	zone_shake(zone, km_sleepflag) -- standard shake routine
 */
...
extern int	zone_shake(zone_t *, int);

#if NUMA_BASE
extern void	*kmem_zone_alloc_node_hint(zone_t *, cnodeid_t, int);
extern void	*kmem_zone_zalloc_node_hint(zone_t *, cnodeid_t, int);
extern void	 kmem_zone_set_default_node(cnodeid_t);
extern cnodeid_t kmem_zone_get_default_node(void);
#else
#define kmem_zone_alloc_node_hint(z,n,f)	kmem_zone_alloc(z,f)
#define kmem_zone_zalloc_node_hint(z,n,f)	kmem_zone_zalloc(z,f)
#define kmem_zone_set_default_node(n)		
#define kmem_zone_get_default_node(n)		CNODEID_NONE
#endif /* NUMA_BASE */
 ...
 *	The clients can pre-fill the zones and zones never get shaken (trimmed)
 ...
 *	kmem_zone_fill_max(zone, pointer-to-memory, size-of-memory,
 *		max-pages-to-allocate, return-code) -- pre-fill, but not
 *		if it will exceed max-pages-to-allocate;
 *	kmem_zone_reserve(zone, number-of-items-wanted) -- pre-fill.
 *	kmem_zone_reserve_node(cnodeid_t, zone, number-of-items-wanted)
 *		pre-fill zone on designated node with this number of items.
 */
...
extern void	kmem_zone_fill_max(zone_t *, void *, int, int, int *);
...
extern void	shake_register(int, int (*)(int, cnodemask_t));
extern int	shake_shake(int, cnodemask_t);

#define SHAKE_DECL(x)		int x(int, cnodemask_t)
#define SHAKE_FUNCTION(x,y,z)	int x(int y, cnodemask_t z)
#define SHAKE_SHAKE(x)		shake_shake(x,CNODEMASK_FROM_NUMNODES(numnodes))
#define SHAKE_SHAKE_NODES(x,y)	shake_shake(x, y)
...
#define SHAKE_DECL(x)		x(int)
#define SHAKE_FUNCTION(x,y,z)	x(int y)
#define SHAKE_SHAKE(x)		shake_shake(x)
#define	SHAKE_SHAKE_NODES(x,y)	shake_shake(x)
