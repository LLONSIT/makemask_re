...
typedef struct mrlock_s {
	...
	union {
		struct {
			short	qcount; /* # threads in the actual queue */
			short	qflags;
		} mr_st;
		uint_t	qbits;
	} mr_un;
 ...
 * Default mrlock behavior is partially controlled by the mrlock_behavior
 * systune.  If a behavior is desired / not - desired for a specific
 * lock it should be set with the following flags.
 */
#define MRLOCK_BARRIER		0x1
#define MRLOCK_BEHAVIOR		0x2
#define MRLOCK_DBLTRIPPABLE	0x4
#define MRLOCK_ALLOW_EQUAL_PRI	0x8
#define	MRLOCK_DIVEST_PRI_INHERIT	0x10
#define MRLOCK_BACKOFF		0x20 /* High contention backoff lock */
#define MRLOCK_NOBARRIER	0x40 /* Do not use barrier mode */
#define MRLOCK_NOEQUALPRI	0x80 /* Do not use equal pri mode */

#define MRLOCK_DEFAULT	0

#ifdef _KERNEL
#define mrinit(mrp, name)	mrlock_init(mrp, 0, name, -1)
#define mrbhinit(mrp, name)	mrlock_init(mrp, MRLOCK_BARRIER|MRLOCK_BEHAVIOR|MRLOCK_DBLTRIPPABLE|MRLOCK_DIVEST_PRI_INHERIT|MRLOCK_NOEQUALPRI, name, -1)
...
extern void	mrdivest(mrlock_t *);
extern int	mrlock_mine(mrlock_t *, struct kthread *);
extern int	mrpromote_or_unlock(mrlock_t *mrp);	/* Atomically try
							 * READ -> WRITE or
							 * unlock */
...
extern int mr_promote_or_unlock(mrlock_t *);
 ...
 * time remaining.
 */
extern int sv_bitlock_timedwait(sv_t *, int, uint *, uint, int, int,
...
extern void sv_timedwait(sv_t *, int, void *, int, int,
					timespec_t *, timespec_t *);
extern int sv_timedwait_notify(sv_t *, int, void *, int, int,
					timespec_t *, timespec_t *);
