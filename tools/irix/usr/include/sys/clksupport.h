...
#define DISABLE_TMO_INTR()	if  (cputolocalslice(cpuid())) \
...
/*
 * The Real Time Clock in the bedrock chip is incremented once for every
 * 2*(RT_MAX_COUNT+1) cycles of the bedrock clock.
 */
#define RT_MAX_COUNT		(signed)((LOCAL_HUB_L(LB_RT_LOCAL_CTRL) & \
				  LRLC_MAX_COUNT_MASK) >> LRLC_MAX_COUNT_SHFT)
#define BEDROCK_MHZ		200	/* bedrock clock in MHz */
#define NSEC_PER_CYCLE		(1000/BEDROCK_MHZ*2*(RT_MAX_COUNT+1))
#define CYCLE_PER_SEC		(NSEC_PER_SEC/NSEC_PER_CYCLE)
#define CLK_FCLOCK_FAST_FREQ	(CYCLE_PER_SEC/1000)
#endif
