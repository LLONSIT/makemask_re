...
extern mac_label *mac_importlabel(mac_label *);
...
extern void  mac_private_zone_init  (void);
extern void *mac_private_zone_alloc (int flags);
extern void  mac_private_zone_free  (void *ptr);

extern void  mac_private_lock_init  (void);
extern int   mac_private_lock       (void);
extern void  mac_private_unlock     (int c);
...
#define	_MAC_IMPORTLABEL(a)	((mac_enabled)? mac_importlabel(a): NULL)
...
extern int mac_chkmld_vsetlabel( struct vnode *, mac_label *, int *);
#define _MAC_CHKMLD_VSETLABEL(v,m,e) \
	(mac_enabled? (mac_never(), mac_chkmld_vsetlabel(v,m,e)): 0)

extern int mac_vsetlabel_chkrm( struct vnode *, char *name, struct vnode *, struct cred *, int *);
#define _MAC_VSETLABEL_CHKRM(d,n,v,c,e) \
	(mac_enabled? (mac_never(), mac_vsetlabel_chkrm(d,n,v,c,e)): 0)
...
extern mac_label *mac_getdft_lp( struct vnode * );
#define _MAC_GETDFT_LP(v) \
	(mac_enabled? (mac_never(), mac_getdft_lp(v)): NULL)
...
extern int mac_udf_iaccess(struct udf_inode *, mode_t, struct cred *);
#define _MAC_UDF_IACCESS(i,m,c) \
	(mac_enabled? (mac_never(), mac_udf_iaccess(i,m,c)): 0)
...
extern int mac_spec_attr_set( struct bhv_desc *, char *, char *, int, int,
			     struct cred *);
#define _MAC_SPEC_ATTR_SET(v,n,a,b,f,c) \
	(mac_enabled? (mac_never(), mac_spec_attr_set(v,n,a,b,f,c)): -1)
