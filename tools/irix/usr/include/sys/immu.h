...
typedef struct pte {
	__uint64_t
		pte_uncattr: 2,	/* HW: PTE uncached attributes */
                ...
                pte_pgmaskshift: 4, 	/* SW: pfn page size field */	
		pte_rmap_index:16,	/* SW: rmap index field */
		pte_reserved: 4,	/* Unused bits at this time */
		pte_nr : 3,	/* SW: page reference count */
		pte_dirty : 1,	/* SW: page dirty bit */
		pte_sv : 1,	/* SW: page valid bit */
		PTE_PFN(26),	/* HW: core page frame number or 0 */
...
#define NRMASK          0x0000007000000000
#define NRSHIFT         36
#define RESVDMASK       0x0FFFFFE000000000

#else /* SN0XXL || SN1 */

/* this is SN0 and IP30 */
#define PFNDWIDTH	26
#define NPFNMASK        0x00000000FFFFFFC0
#define NRMASK          0x0000001C00000000
#define NRSHIFT         34
...
#else
#define BASETABSHIFT64		6
#define BASETABSHIFT		6
#endif /* defined(SN0XXL) || defined(SN1) */
#else 
#define BASETABSHIFT64		5
...
#define BPBASETABSHIFT		(BPSSHIFT+BASETABSHIFT64)
...
#define NDREF		3		/* positionless count */
...
#if defined (SN0XXL) || defined(SN1)
#define PG_NDREF	0x7000000000UL	/* reference count (software) */
...
#elif defined(SN0)
#define PG_NDREF        0x1C00000000UL  /* reference count (software) */
...
#ifdef SN 
/* 
 * Uncached Attribute available on SN -- SN has to use 
...
#define	mkpde(mode,pfn)	(mode | ((paddr_t)(pfn) << PTE_PFNSHFT))
...
#define LOWDEFATTACH	((uvaddr_t)0x4000000L)	/* leave the first segment empty by */
						/* default */
/*
 * 64-bit executables formerly attached at 1/4 TB (0x4000000000UL) as
 * a first choice, and as a side effect almost always landed there.  This
 * is bad since executables on today's larger machines may very well
 * grow the heap and run into the mmap.  However, there is usually some
 * useful space below the heap, so LOWDEFATTACH64 was changed to match
 * the value for 32-bit executables and thus we will use that space first.
 */
#define LOWDEFATTACH64	LOWDEFATTACH
 ...
 * attribute bits.
 */
#if _MIPS_SIM == _ABI64 && (defined(R10000) && defined(SN))
/*
...
#else
#if IP20 || IP22 || IP26 || IP28 || IP30 || SN
 ...
 *
 * These fields are specific to SN.
