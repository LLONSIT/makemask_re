...
/* ioctl code for keyboard and mouse drivers */
#define	PCKM_IOC(x)		(('p' << 8) | (x))
#define	PCKMHWG			PCKM_IOC(1)	/* create hwgraph path alias */
...
/* Mouse/Keyboard id numbers */
#define MOUSE_ID        0x00		/* Standard 3-button mice */
#define WHEELMOUSE3_ID	0x03		/* 3-button wheel mice */
#define WHEELMOUSE5_ID	0x04		/* 5-button wheel mice */

#define KEYBD_ID     	0x83
#define KEYBD_BEEP_ID1 	0x53
#define KEYBD_BEEP_ID2 	0x47
#define KEYBD_USB	0x80			/* generic usb kbd */
#define KEYBD_USB_BEEP	0x81			/* usb kbd w/beep */
...
#define CMD_SCALE11	0xe6	/* KBD_ACK's and enters 1:1 scaling mode */
#define CMD_SCALE21	0xe7	/* KBD_ACK's and enters 2:1 scaling mode */
#define CMD_MSRES	0xe8	/* ack's, reads new resolution byte, ack's */
#define CMD_MSSTAT	0xe9	/* KBD_ACK's and sends a status packet */
#define CMD_STREAM	0xea	/* KBD_ACK's and enters stream (normal) mode */
#define CMD_READ	0xeb	/* KBD_ACK's and sends a movement packet */
#define CMD_RESETWRAP	0xec	/* KBD_ACK's and enters previous mode */
#define CMD_SETLEDS	0xed
#define CMD_WRAPMODE	0xee	/* KBD_ACK's and enters wrap (echo) mode */
#define CMD_SELSCAN	0xf0	/* KBD_ACK's and enters remote (poll) mode */
#define CMD_ID	0xf2	/* KBD_ACK's and send its ID */
#define CMD_MSSAMPL	0xf3	/* ack's, reads new sample rate byte, ack's */
#define CMD_ENABLE	0xf4	/* KBD_ACK's and enables data reporting */
#define CMD_DISABLE	0xf5	/* KBD_ACK's and disables data reporting */
#define CMD_DEFAULT	0xf6	/* Default mode, Sampl=100, Res=4/mm, Scl=1:1 */
#define CMD_ALLMB	0xf8
#define CMD_ALLMBTYPE	0xfa
#define CMD_KEYMB	0xfc
#define CMD_RESEND	0xfe	/* Requests the mouse resends its data */
#define CMD_MSRESET	0xff	/* KBD_ACK's & enters reset (power-on) mode */
...
/* Textport console keyboard/mouse support driver functions
 */
typedef struct pckm_func_list      	pckm_func_list_t;
struct pckm_func_list {
	void (*early_pckminit)(void);		/* early_pckminit() */
	 int (*pckm_kbdexists)(void);		/* pckm_kbdexists() */
	void (*pckm_setleds)(int);		/* pckm_setleds() */
	void (*pckm_bell)(void);		/* pckm_bell() */
	void (*gl_setporthandler)(int, int (*)(unsigned char));
						/* gl_setporthandler() */
	 int (*du_keyboard_port)(void);		/* du_keyboard_port() */
};

void   pckm_console_register(pckm_func_list_t *);
...
#define MOUSE_GETID 0x12	/* Magic # to probe the mouse ID */

/* pcmouse_mode systune defines */
#define PCMOUSE_MODE_DFLT	0	/* Default */
#define PCMOUSE_MODE_3BTN	1	/* Wheel is buttons 4 & 5 */
#define PCMOUSE_MODE_5BOTH	2	/* 5 btns, wheel & buttons gen 4&5 */
