...
/* the pragma forces gthreadlockedlist_t to be cache aligned */
typedef struct gthreadlockedlist_s {
    gfxtablelock_t	mrlock;
    gthread_t		*list;
} gthreadlockedlist_t;
#if defined(MP)
#pragma set type attribute gthreadlockedlist_t align=128
#endif
...
extern gthreadlockedlist_t *gthread_table;
 ...
 */
#if !defined(IP27) && !defined(IP30) && !defined(IP32) && !defined(IP35)
 ...
 *   struct gfx_gfx is 256 bytes on SN1
 *   cpumask_t is 64 bytes on SN1
 */

typedef struct gfx_gfx {
	lock_t       gx_lock;		/* for mutual exclusion of gx data */
...
#if defined(MP)
#pragma set type attribute gfx_gfx_t align=128
#endif

#undef GFXP_WITH_SPINLOCKS
#ifdef GFXP_WITH_SPINLOCKS
#define GFXP_LOCK(p)		mutex_spinlock(&(p)->gx_lock)
#define GFXP_UNLOCK(p,s)	mutex_spinunlock(&(p)->gx_lock,s)
#else
#define GFXP_LOCK(p)		disableintr()
#define GFXP_UNLOCK(p,s)	enableintr(s)
#endif
#define GFXP_SET(p,x)		(p)->gx_flags |= (x)
#define GFXP_UNSET(p,x)		(p)->gx_flags &= ~(x)
#define GFXP_TEST(p,x)		(p)->gx_flags & (x)
...
extern void GfxAddHwgraphEdge(vertex_hdl_t, struct gfx_data *);
...
#if defined(IP19) || defined(IP21) || defined(IP25) || defined(IP27) || defined(IP30) || defined(IP35)
#define PRE_GET_TIME
#define GET_TIME  GET_LOCAL_RTC
#elif defined(IP20) || defined(IP22) || defined(IP26) || defined(IP28) || defined(IP32)
