...
extern void tlbpid_use(struct utas_s *, tlbpid_t, int);
...
typedef struct ut_csaacct_s {
	accum_t		csa_vmem1;	/* virtual memory integral 1 */
	accum_t		csa_vmem2;	/* virtual memory integral 2 */
	accum_t		csa_vmem3;	/* virtual memory integral 3 */
	accum_t		csa_mem2;	/* physical memory integral 2 */
	accum_t		csa_mem3;	/* physical memory integral 3 */
	accum_t		csa_vhimem;	/* highwater mark for virtual memory */
	accum_t		csa_pgswaps;	/* # of pages swapped in/out */
} ut_csaacct_t;	

struct itimerlast {
	struct timeval itl_usr;	/* itimer last update user time */
	struct timeval itl_sys;	/* itimer last update system time */
};
...
typedef struct uthread_s {
	...
	uchar_t		ut_nonpreempts;	/* Failed preemptions */
...
#if defined(IP19) || defined (IP21) || defined(IP25) || defined(IP27)
	void		*ut_share_pad;	/* Free for use */
#endif /* IP19 || IP21 || IP25 || IP27 */
...
#define NWIREDJUMP	3
	...
	ut_csaacct_t	ut_csaacct;	/* CSA accounting info */

	/*
	 * Holds the ktimer values the last time the itimer
	 * was decremented.
	 */
	struct itimerlast *ut_timerlu;
...
extern int	uthread_csaacct(uthread_t *, void *);
extern void	blockme(void);
extern int	blockset(uthread_t *, int, int, int, int);
