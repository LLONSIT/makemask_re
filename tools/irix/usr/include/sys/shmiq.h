...
#ifndef NO_SGIMULTICURSOR_SHMIQ
struct rect {			/* boundaries used for cursor offsets */
	int x ;
	int y ;
	unsigned int w ;
	unsigned int h ;
...
#ifndef NO_SGIMULTICURSOR_SHMIQ
        int hasOffset ;		/* per pipe flag to indicate offset validity */
        struct rect offset ;	/* x and y cursor offsets for a given pipe */
#endif
} ;
...
#ifndef NO_SGIMULTICURSOR_SHMIQ
	int useMultiCursor ;		/* shmiq-local toggle for multicursor */
...
#ifndef NO_SGIMULTICURSOR_SHMIQ
struct shmiqsetcoff {	/* for QIOCSETCOFF */
	short	x ;
	short	y ;
	short	w ;
	short	h ;
...
#ifndef NO_SGIMULTICURSOR_SHMIQ
#define	QIOCSETCOFF	_IOW('Q',12,struct shmiqsetcoff ) /* set cursor offsets for a pipe */
#define	QIOCSETCPALL	_IOWR('Q',13,struct shmiqsetcpos ) /* set all cursor positions */
#endif
