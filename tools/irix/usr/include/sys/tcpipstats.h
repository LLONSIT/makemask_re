...
struct	ipstat {			/* IP statistics */
	...
	__uint64_t	ips_strictreassoverlapfrags;	
	/* # fragments dropped because they overlap with others in the 
	 * reassembly queue (only incremented when ipv4_strict_reassembly 
	 * is on) */
	__uint64_t	ips_strictreassgapfrags;	
	/* # fragments dropped because they introduced a gap in the 
	 * reassembly queue (only incremented when ipv4_strict_reassembly 
	 * is on) */
};
...
struct	ip6stat {
	...
	__uint64_t	ip6s_ofragments;/* output fragments created */
        __uint64_t      ip6s_cantfrag;  /* don't fragment flag was set, etc. */
        __uint64_t      ip6s_badoptions;/*error in option processing */
...
/*
 * IP Striping specific statistics
 */
struct isstat {
	__uint64_t	iss_in_window;		/* input in window */
	__uint64_t	iss_in_underflow;	/* input underflow */
	__uint64_t	iss_in_overlap;		/* input overlap */

	__uint64_t	iss_up_disordered;	/* packets up disordered */
	__uint64_t	iss_up_ordered;		/* packets up ordered */

	__uint64_t	iss_outq_full;		/* outq is full */
	__uint64_t	iss_outq_wakeups;	/* outq thread wakeups */
	__uint64_t	iss_outq_drains;	/* outq thread executions */

	__uint64_t	iss_reorder_wakeups;	/* reorder thread wakeups */
	__uint64_t	iss_reorder_drains;	/* reorder thread executions */

	__uint64_t	iss_drain_underflow;	/* uflow q drains */
	__uint64_t	iss_drain_loop;		/* window !empty after drain */
	__uint64_t	iss_drain_empty;	/* window empty before drain */

	__uint64_t	iss_window_stalls;	/* window stalls detected */
	__uint64_t	iss_window_flush_null;	/* flush null (empty) slot */
	__uint64_t	iss_window_seqno_fixup;	/* flush null (empty) slot */
	__uint64_t	iss_window_flush_skipped; /* skipped !empty slot */
	__uint64_t	iss_window_flush_nlinks; /* flushed all links once */

	__uint64_t	iss_link_quota_oflows;	/* link quota filled */
	__uint64_t	iss_link_empty_headers;	/* Zero length leading mbuf */
	__uint64_t	iss_link_header_allocs;	/* Header mbuf allocs */
	__uint64_t	iss_link_soft_cksums;	/* payload software cksums */
	__uint64_t	iss_link_sync_seqno;	/* link seqno sync recv'd */

	__uint64_t	iss_err_bad_version;	/* unknown version in pkt */
	__uint64_t	iss_err_input_no_link;	/* no link matching pkt */
...
#define ISSTAT(x)		((struct kna2 *)(private.knaptr))->isstat.x++
#define ISSTAT_ADD(x, c)	((struct kna2 *)(private.knaptr))->isstat.x +=c
...
#define _KA_STPCB_HEAD		27
#define _KA_LOOKUPX_TIMING	28
#define	_KA_MAX			29
