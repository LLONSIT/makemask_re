/*
 *    Unpublished copyright (c) 1987-1997,1999-2005 Silicon Graphics, Inc.
 ...
 * Defines for number of SCSI buses
   NOTICE:  SCSI_MAXTARG has been removed from the kernel.  Its defines
   are deprecated and remain for older kernels and user code compatability.
 */
#if defined(EVEREST)
...
#if defined(IP30) || defined(SN) || defined(IP32)
#define SCSI_MAXTARG	  	127
...
#define SRH_MULTCMDS  0x1000    /* HBA handles multiple cmds on its own */
#define SRH_ATAPI     0x2000    /* is ATAPI (IDE) device. mode 10 byte
				 * cmds only, no settable blksize */
...
/*
 * Definition of array element filled in by SOP_GETMAP.  Call SOP_GETMAP_SZ
 * to get number of elements returned by SOP_GETMAP.  
 */
struct fc_targ_map {
	uint32_t	port_id;
	uint32_t	reserved;
...
/*
 * Structures used by SOP_GET_LNK_STS
 */
struct link_status {
	uint32_t	link_failure_count;
	uint32_t	loss_of_sync_count;
	uint32_t	loss_of_signal_count;
	uint32_t	primitive_sequence_protocol_error_count;
	uint32_t	invalid_transmission_word_count;
	uint32_t	invalid_crc_count;
	uint64_t 	reserved;
};
typedef struct link_status link_status_t;

struct port_link_statistics {
	uint64_t 	id;		/* loop id/port name */
	uint64_t 	name;		/* node name */
	link_status_t	link_status;
	int32_t 	topology;	/* topology */
	uint32_t 	reserved[3];
};
typedef struct port_link_statistics port_link_statistics_t;

/* FC topologies */
#define LOOP_TOPOLOGY       1     /* FC-AL topology          */
#define SWITCH_TOPOLOGY     2     /* Fabric topology         */
#define P2P_TOPOLOGY        3	  /* point-to-point topology */

/* link statistics options */
#define GET_LNK_STATS_MAP_SZ            0       /* get max target count          */
#define GET_CTLR_LNK_STATS              1       /* controller link statistics    */
#define GET_LOOP_TARG_LNK_STATS         2       /* loop target link statistics   */
#define GET_FABRIC_TARG_LNK_STATS       3       /* fabric target link statistics */
#define GET_ALL_TARGS_LNK_STATS         4       /* all targets link statistics   */

struct fw_info
{	uint64_t	fw_info;	/* fw identification */
	uint64_t	hw_info;	/* board identification */
};
...
#define SOP_ENABLE		_SCIO(9)   /* enable a controller to execute commands */
#define SOP_DISABLE		_SCIO(10)  /* disable a controller from executing commands */
...
#define SOP_GETMAP_SZ	 _SCIO(26)	   /* retrieve the target map max element count*/
#define SOP_GETMAP	 _SCIO(27)	   /* retrieve the ctlr's target map */
#define SOP_GET_LNK_STS  _SCIO(28)         /* get link error statistics */
#define SOP_CTLR_STATE	 _SCIO(29)	   /* get controller state information */
#define SOP_HBA		 _SCIO(30)	   /* execute an hba api command */
		/* sb_arg:  command */
		/* sb_opt:  unused */
		/* sb_addr: data buffer address */
#define		HBA_get_adapter_attributes		1
#define		HBA_get_adapter_port_attributes		2
#define		HBA_get_port_statistics			3
#define		HBA_get_discovered_port_attributes	4
#define		HBA_refresh_information			5
#define		HBA_get_fcp_target_mapping		6

/* for ATAPI / IDE */
#define SOP_BLKSZ        _SCIO(31)         /* set/get blocksize */

#define SOP_FW_DOWNLOAD  	_SCIO(33)  /* download fw to LSI board */
#define SOP_GET_FW_VERSION	_SCIO(34)  /* fetch LSI firmware info */
#define SOP_GET_WWN		_SCIO(35)  /* fetch fc hba world wide name */

/* SOP_CTLR_STATE result for qlfc driver */
/*
** Note: this is a bit field and should be tested by examining
** the specified bit below.  Other bits are undefined and should
** NOT be presumed to be zero!
*/
#define	CTLR_ENABLED		0x00000001	/* see SOP_ENABLE/SOP_DISABLE */
...
#define TARG_GID_NUM_FORMAT             "target/%llx"
...
typedef uint64_t fw_targ_t; /* type for 1394 (global ID) */
typedef vertex_hdl_t scsi_pci_vhdl_t;
...
typedef struct scsi_ctlr_info {
	...
	uchar_t				sci_adap;	/* adapter number */	
        uchar_t                         sci_scsi_type;  /* type of interface */
	uint8_t				ref_count:7,
					ref_flag:1;
	uint8_t				reserved;
	/* maintains offset to sci_info and avoids wasting 8 bytes */
#if _MIPS_SZPTR == 64
	int32_t				sci_alias_num;	/* "SCSI bus number" from ioconfig */
#endif
	void				*sci_info;	/* info maintained for hba driver */
#if _MIPS_SZPTR == 32
	int32_t				sci_alias_num;	/* "SCSI bus number" from ioconfig */
#endif
} scsi_ctlr_info_t;
...
#define SCI_SCSI_TYPE(p)		(p->sci_scsi_type)

#define SCI_INFO(p)			(p->sci_info)

#define SCI_SCSI_TYPE_UNSPECIFIED    0    /* unspecified */
#define SCI_SCSI_TYPE_1394           1    /* firewire, ie SBP2 on OHCI over 1394 */
#define SCI_SCSI_TYPE_PARALLEL       2    /* parallel scsi */
#define SCI_SCSI_TYPE_FC             3    /* fibre channel */
#define SCI_SCSI_TYPE_SAS            4    /* sas/sata */
...
#define SLI_SCSI_TYPE(p)	(SCI_SCSI_TYPE(STI_CTLR_INFO(SLI_TARG_INFO(p))))
                                                                        /* controller number
									 * from lun info ptr
									 */
...
#define SDEVI_SCSI_TYPE(p)	(SLI_SCSI_TYPE(SDEVI_LUN_INFO(p)))
...
#define SDI_SCSI_TYPE(p) 	(SCI_SCSI_TYPE(STI_CTLR_INFO(SLI_TARG_INFO(SDI_LUN_INFO(p)))))
...
extern void scsi_ctlr_info_free(vertex_hdl_t 	vhdl);
...
/*
 * Lock defines for access of vertices by the device drivers (dksc,
 * ds, tpsc, smfd, 3rd party), and removal of vertices by the hba
 * drivers.
 */
extern	mrlock_t		scsi_hwgraph_lock;
#define SCSI_HWG_ACCESS()	mraccess(&scsi_hwgraph_lock);
#define SCSI_HWG_UPDATE()	mrupdate(&scsi_hwgraph_lock);
#define SCSI_HWG_UNLOCK()	mrunlock(&scsi_hwgraph_lock);
#define SCSI_HWG_UNUPDATE()	mrunlock(&scsi_hwgraph_lock);
#define SCSI_HWG_UNACCESS()	mrunlock(&scsi_hwgraph_lock);
