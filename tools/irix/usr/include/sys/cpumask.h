...
#define CPUMASK_ORNOTM(p, q)	(p)._bits[0] |= ~((q)._bits[0]),	\
				(p)._bits[1] |= ~((q)._bits[1])
#define CPUMASK_COPY(p, q)	(p)._bits[0] = ((q)._bits[0]),         \
				(p)._bits[1] = ((q)._bits[1])
...
#define	CPUMASK_ATOMSET(p,q)	{					\
	...
	for (_i = 0 ; _i < CPUMASK_SIZE ; _i++) {				\
		if ((q)._bits[_i]) {					\
			atomicSetCpumask((cpumask_t *)&(p)._bits[_i],	\
				 	 (cpumask_t *)&(q)._bits[_i]);	\
			break;						\
		}							\
	}								\
}
...
#define	CPUMASK_ATOMCLR(p,q)	{					\
	...
	for (_i = 0 ; _i < CPUMASK_SIZE ; _i++) {				\
		if ((q)._bits[_i]) {					\
			atomicClearCpumask((cpumask_t *)&(p)._bits[_i],	\
					   (cpumask_t *)&(q)._bits[_i]);	\
			break;						\
		}							\
	}								\
...
static __inline int CPUMASK_IS_ZERO (cpumask_t p)
{
	int i;
...
static __inline int CPUMASK_IS_NONZERO (cpumask_t p)
{	
	int i;
...
static __inline int CPUMASK_NOTEQ (cpumask_t p, cpumask_t q)
{
	int i;
...
static __inline int CPUMASK_TSTM (cpumask_t p, cpumask_t q)
{
	int i;
