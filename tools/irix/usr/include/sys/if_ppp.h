...
#    define PL_ST_RX_IP	    0x000200	/* IP input ok */
#    define PL_ST_TX_IP6    0x000400	/* IPv6 output ok */
#    define PL_ST_RX_IP6    0x000800	/* IPv6 input ok */
...
/* returned by SIOC_PPP_INFO2 */
struct ppp_info2 {
    __int32_t	    pi_version;
#    define	PI2_VERSION 1
    __int32_t	    pi_len;		/* sizeof(struct ppp_info) */
    struct ppp_bundle pi_bundle;
    struct {
	__int32_t   tx_states;		/* # of TX SLOTS */
...
#ifdef _KERNEL
#define SIOC_PPP_INFO	_IOWR('P',15,__uint32_t)    /* obsolete - for backward 
						       compatability */
#define SIOC_PPP_INFO2	_IOWR('P',16,__uint32_t)    /* what's happening */
#else
#if _MIPS_SIM == _ABI64
/* this interface is only for 32-bit programs */
#else
#define SIOC_PPP_INFO	_IOWR('P',15,struct ppp_info*)
#define SIOC_PPP_INFO2	_IOWR('P',16,struct ppp_info*)
#endif
#endif /* KERNEL */
#define SIOC_PPP_IP_RX_OK _IOWR('P',16,struct ppp_arg)	/* enable IP RX */
#define SIOC_PPP_IP6_TX_OK _IOWR('P',17,struct ppp_arg)	/* enable IPv6 TX */
#define SIOC_PPP_IP6_RX_OK _IOWR('P',18,struct ppp_arg)	/* enable IPv6 RX */
