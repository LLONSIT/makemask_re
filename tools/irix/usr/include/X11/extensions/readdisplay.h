...
#define X_ShmReadDisplayRects		2

#ifdef _READDISPLAY_SERVER_

extern int rdScreenPrivateIndex;
...
typedef struct _ShmReadDisplayBuf {
    int width, height;		/* size of the buffer */
    XShmSegmentInfo *shminfo;	/* shared memory info */
    int offset;		/* offset in the buffer to the start of the data */
} ShmReadDisplayBuf;
...
extern ShmReadDisplayBuf *XShmCreateReadDisplayBuf(
#if NeedFunctionPrototypes
	Display *	/* dpy */,
	char *		/* data */,
...
extern Status XShmReadDisplayRects (
#if NeedFunctionPrototypes
	Display *	/* dpy */,
	Window	 	/* srcwin */,
	XRectangle *	/* pRects */,
...
extern void XShmDestroyReadDisplayBuf (
#if NeedFunctionPrototypes
	ShmReadDisplayBuf * /* shmbuf */
#endif
);
