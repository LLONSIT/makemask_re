#ifndef __XTRAPDI__
#define __XTRAPDI__
/* RCSID = $Header: /plroot/x/irix6.5f/.RCS/PL/xc/include/extensions/xtrap/RCS/xtrapdi.h,v 1.1 1992/10/02 20:21:21 hohn Exp $" */
...
typedef struct /* used by XEConfigRequest */
{
    ...
    CARD8       cmd_key B8;       /* Keyboard command_key (KeyCode) */
/* 
...
{
    CARD8    type B8;                 /* (must be first) as in XEvent */
    CARD8    detail B8;               /* Detail keycode/button as in XEvent */
    CARD8    screen B8;               /* screen number (0 to n) */
    CARD8    pad B8;                  /* pad to longword */
...
typedef struct 
{
    ...
    CARD8       config_cmd_key B8;       /* Keyboard command_key (KeyCode) */
/* 
 * cmd_key is intentionally *not* defined KeyCode since it's definition is 
 * ambiguous (int in Intrinsic.h and unsigned char in X.h.
 */
...
typedef struct
{
    ...
    Mask    event_mask B32;  /* The event mask for our base event */
    CARD16  pad B16, pad2 B16, pad3 B16;         /* obsolete field */
...
typedef struct 
{
    ...
    CARD8       data_config_cmd_key B8;       /* Keyboard cmd_key (KeyCode) */
/* 
 * cmd_key is intentionally *not* defined KeyCode since it's definition is 
 * ambiguous (int in Intrinsic.h and unsigned char in X.h.
 */
...
{   /* this must be quadword aligned for portability */
    ...
    CARD8    type B8;                /* event id, request id, special id */
    CARD8    screen B8;              /* screen number (0 to n) */
