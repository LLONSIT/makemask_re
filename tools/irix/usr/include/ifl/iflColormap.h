...
#ifndef _iflColormap_h_
#define _iflColormap_h_

class iflColormap : public iflLut {
...
public:
	...
	: iflLut(numChan, dtype, min, max) {}

    // external api: name=iflColormapNoAlloc
    iflColormap(void* table, int numChan, iflDataType dtype, int min, int max)
	: iflLut(table, numChan, dtype, min, max) {}
