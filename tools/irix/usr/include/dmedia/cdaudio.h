...
struct cdcontroltype { unchar control:4, type:4; };
...
typedef CDSUBCODEQ TOSHIBA_CDSUBCODEQ;

/*
 * Structure of CD subcode Q on the Pansonic drives
 */
typedef struct panasonic_subcodeQ {
    struct cdcontroltype controltype;
    union {
        struct {
	    struct cdpackedbcd track;
	    struct cdpackedbcd index;	/* aka point during track 0 */
...
typedef struct panasonic_cdframe {
    char audio[CDDA_DATASIZE];
    struct panasonic_subcodeQ subcode;
} PANASONIC_CDFRAME;
...
int CDreaddapanasonic( CDPLAYER *cd, PANASONIC_CDFRAME *buf, int num_frames );
/*
...
void CDparseframepanasonic(CDPARSER *cdp, PANASONIC_CDFRAME *cdfp);
