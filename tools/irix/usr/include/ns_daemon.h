...
#define NSD_NOCALLOUT	16	/* For use with nsd_loopack with NSD_LOOP_SKIP flag set */
...
#define NSD_FLAGS_DYNAMIC (1<<1) /* Directory was dynamically created */
#define NSD_FLAGS_GRPBYMEMBER (1<<2) /* Request for the group.bymember map */
#define NSD_FLAGS_POSTPROC (1<<3) /* This req is to be post processed */
#define NSD_FLAGS_RECURSIVE (1<<4) /* This a recursive lookup */
...
int nsd_cred_new_from_rpc(struct opaque_auth *, nsd_cred_t **, 
...
/* PRINTFLIKE2 */
void nsd_logprintf(int, const char *format, ...);
void nsd_shake(int);
...
/*
GROUP.BYMEMBER NOTES:

Files in the group.bymember table have the following format:
    key: grpid1, grpid2, grpid3, ...

The list of group IDs is the union of the group IDs returned for "key"
by all of the callout libraries defined for the group map.

The nsd_append_group_bymember function should be used to append
group IDs to the file data returned by the group.bymember table. This
function prepends the "key:" format if it does not exist, and appends
the passed comma-separated list of group IDs, eliminating duplicates.
Callout libraries should *not* use the nsd_set_result family of functions
to return data for a group.bymember request.

A callout library can test whether a request is from the group.bymember
table by checking for the NSD_FLAGS_GRPBYMEMBER in f_flags. If it is set,
the callout library should use nsd_append_group_byname to add data to
the request, and should unconditionally return NSD_NEXT from its lookup
function.
*/

int
nsd_append_group_bymember(nsd_file_t *    rq,
                          int             status,
                          char *          groups,
                          int             len);

#if defined(_COMPILER_VERSION) && (_COMPILER_VERSION >= 721)
#pragma optional nsd_append_group_bymember
#pragma optional nsd_cred_new_from_rpc
#endif

#ifdef SGI_COMPILING_NSD
/* Structures for minus list (processing of '-' entries in passwd and group: 
 * The following declarations are for NSD internal use only and are subject 
 * to change without notice. They must not be used by non NSD modules.
 */
...
struct minus_node {
	...
	size_t namelen;
        char *gid;
	size_t gidlen;
        struct minus_node *next;
};

/* We save the last modify time of the file when we load the minus list (if any)
 * and stat the file each call to see if the mtime is more recent, in which
 * case we rescan for minus entries.
 */
struct minus_head {
        timespec_t lmtime;      /* last time the file was last modified */
	int update_locked;	/* Because updating this list can involve
				 * recursive lookups, we need to be able to
				 * lock it during update if recursion is necessary.
				 * If a updater finds the list locked, it should
				 * register a timeout and retry.
				 */ 
        struct minus_node *list;
};

int postproc_results(nsd_file_t *);

#endif /* SGI_COMPILING_NSD */
