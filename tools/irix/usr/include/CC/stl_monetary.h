				       ...
				       bool&    __syntax_ok)
{
  ...
  while (__first != __last                        && 
         __digits < __frac_digits                 &&
         __ctype.is(ctype_base::digit, *__first)) {
    *__out++ = *__first++;
    ++__digits;
  }
  __syntax_ok = !__ctype.is(ctype_base::digit, *__first) &&
                __digits == __frac_digits;  
  return make_pair(__first, true);
}
...
_InputIter money_get<_CharT, _InputIter>
  ...
  bool __symbol_required = __str.flags() & ios_base::showbase;
  string_type __buf;
...
ptrdiff_t __insert_grouping(_CharT *, _CharT *, const string&,
			    _CharT, _CharT, _CharT, int);
...
money_put<_CharT, _OutputIter>
 ::do_put(iter_type __s, bool __intl, ios_base& __str,
  ...
  bool __generate_curr = __str.flags() & ios_base::showbase;
  if (__generate_curr)
