...
/***
 ***
 ***  INCLUSION OF THIS FILE IS DEPRECATED.  This is based on an early
 ***  draft of the C++ standard, and it is provided only for backward
 ***  compatibility.  This file WILL BE REMOVED in a future release.
 ***
 ***  This file was written for pre-standard C++, and it IS NOT SUPPORTED
 ***  in standard-conforming (-LANG:std) mode.
 ***
 ***  You should use the header <string> instead.  It contains a fully
 ***  conforming implementation of class basic_string<> from the C++
 ***  standard.
 ***
 ***/
...
__STL_TEMPLATE_NULL struct string_char_baggage<char> {
...
public:
    ...
    friend
    ostream&
    operator<< __STL_NULL_TMPL_ARGS (ostream& o,
                                     const basic_string<charT>& s) _THROW_NONE ;

    friend
    istream&
    operator>> __STL_NULL_TMPL_ARGS (istream& i,
                                     basic_string<charT>& s) _THROW_ALLOC_LENGTH ;

    friend
    basic_string<charT>
    operator+ __STL_NULL_TMPL_ARGS (const basic_string<charT>& lhs,
                                    const basic_string<charT>& rhs)
    _THROW_ALLOC_LENGTH ;

    friend
    basic_string<charT>
    operator+ __STL_NULL_TMPL_ARGS (const charT* lhs,
                                    const basic_string<charT>& rhs)
    _THROW_ALLOC_LENGTH ;

    friend
    basic_string<charT>
    operator+ __STL_NULL_TMPL_ARGS (charT lhs,
                                    const basic_string<charT>& rhs) _THROW_ALLOC_LENGTH ;
...
struct __STD::hash< ::string>
#else
struct hash<string>
#endif
{
  typedef ::string _String;
  size_t operator()(const _String& str) const
  {
    unsigned long h = 0;
    const _String::char_type* s = str.data();
