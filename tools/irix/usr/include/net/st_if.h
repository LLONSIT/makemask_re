 ...
 */
#define ST_STANDARD_PORT  (1<<0)	/* Non-append port */
#define ST_APPEND_PORT    (1<<1)	/* Append-data port */
#define ST_ALLOC_PORT     (1<<2)	/* Driver allocates port */
#define ST_SPECIAL_PORT   (1<<3)	/* not standard & not append (i.e. port 0) */
...
typedef struct {
	...
	uint32_t bypass		 :1,
		 reserved1       :31;
	uint32_t ddq_size;  /* in rx_descriptors */
	uint64_t ddq_addr;  /* destination descriptor Q */

	/*-- ULP Parameters --*/
	uint32_t ulp_lport;
	uint32_t ulp_fport;
	    /* Typically, ulp_cookie is the inpcb * or other
	     * ulp-specific struct to uniquely identify this
	     * connection.  It's opaque to the driver.
	     */
	void    *ulp_cookie;    
	    /* Typically, ulp_upcall is a wrapper for INPCB_HOLD and
	     * INPBC_RELE or similar ulp-specific function.  This is
	     * used prevent a potential race with clear_port where the
	     * expectation is that the cookie will not be valid soon
	     * after clear_port returns.  The first arg is the cookie.
	     * For the second arg, 0 means release, !0 means hold.
	     */
	void   (*ulp_upcall)(void *, int);
...
typedef struct {
	...
	int32_t		  domain;
	void		 *ulp_cookie;	/* inpcb or similar, ulp-specific */
	void		(*ulp_upcall)(void *, int);
} st_rx_desc_t;

/* second arg to ulp_upcall */
#define STP_UPCALL_RELE 0	/* port_mutex must *NOT* be held in driver */
#define STP_UPCALL_HOLD 1	/* port_mutex must be held in driver */
...
#define ST_IFF_ASSIST	   	(1 << 4)  /* adapter has firmware assist */
#define ST_IFF_BYPASS		(1 << 5)  /* adapter supports OS bypass */
	 ...
	 * bit vector of supported bufx sizes. bits 0-7 are not allowed by ST spec.
	 *
	 * Note: the ST stack on IRIX will only use 16k, 64k, 256k, 1m, 4m, & 16m 
	 * 	pages, so only values 14, 16, 18, 20, 22, and 24 are used, if available.
	 */
	uint32_t  r_bufsize_vec;
	uint32_t  l_bufsize_vec;

	/* NOTE: for max_stu and max_block we don't need a minimum value because the
	 * ST spec requires a minimum of 1 byte. 
	 * Each value is specified as a power of 2.
	 */
	
	uint32_t  r_max_stu;		  /* max STU size NIC can send */
	uint32_t  l_max_stu;		  /* max STU size NIC can receive */
	uint32_t  r_max_block;		  /* max block NIC can send */
	uint32_t  l_max_block;		  /* max block NIC can receive */

	uint32_t  max_cts_outstanding;	  /* max CTS's NIC wants outstanding */


	/* Bufx allocator takes as input the bufx_spray array and 
	 * initializes it's state, potentially storing the data structure
	 * off the bufx_table void.
	 */
...
#define ST_MAX_SPRAY 8
	...
	/* output routine if sp_family is PF_STP */
	...
	/* output routine if sp_family is PF_INET */
	int32_t (*if_ip_output)(struct st_ifnet_s *, struct mbuf *, 
				struct sockaddr *, struct rtentry *,
				struct ipsec *ipsec);
} st_ifnet_t;
