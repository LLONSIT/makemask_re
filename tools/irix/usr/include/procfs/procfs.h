 ...
 *
 * Note: Certain libraries, in particular libspypt, take advantage
 * of the definition of PIOC to pass library-specific opcodes in places
 * you would normally expect to see one of the PIOC* opcodes listed
 * below.  In particular, the presence of the 'q' character in the
 * following opcodes cannot be removed without breaking these libraries.
 */
#define PIOC		('q'<<8)
...
#ifdef CKPT
/*
 * 176-232 reserved for checkpoint/restart
 */
#endif
...
#define PIOCGETPREVCTRS	(PIOC|236)	/* not implemented - reserved */
...
#define PIOCGETSN1REFCNTRS     (PIOC|242)  /* get SN1 page ref cnts*/
#define PIOCGETSN1EXTREFCNTRS  (PIOC|243)  /* get SN1 Soft Ext page ref cnts*/
#define PIOCENEVCTRTHREADS     (PIOC|244)  /* start counters per thread */
#define PIOCGETEVCTRTHREADS    (PIOC|245)  /* get the counters per thread */
#define PIOCSETEVCTRTHREADS    (PIOC|246)  /* set the counters per thread */
#define PIOCRELEVCTRTHREADS    (PIOC|247)  /* release the counters per thread */
...
#define PR_CKPT		0x0100	/* process is being signaled for checkpoint */
...
/*
 * Adding the pr_jid field to the prpsinfo struct has discombubalated the
 * structure packing under the o32 and n32 ABIs.  Adding an eight byte field
 * (like pr_jid) causes the compiler to pad the structure out to an eight
 * byte boundary.  The previous structure size was 284 bytes, which is not
 * an eight byte boundary.  Left to it's own devices, the compiler would
 * round the structure size up to 288 bytes.  That would break the ABI.  In
 * order to avoid that, we must explicitly instruct the compiler to use 4 byte
 * packing, but only on the 32 bit ABIs.  The 64 bit ABI is fine as is.  A
 * similar situation applies to irix5_prpsinfo_t.
 *
 */
#if _MIPS_SIM != _ABI64
#pragma pack(4)
...
#if _MIPS_SIM == _ABI64
	jid_t	pr_jid;		/* job id */
	int64_t	pr_fill1[1];	/* spares */
	int64_t	pr_fill[8];	/* spares */
#else
/*
 * Once we've instructed the compiler to use 4 byte packing, we have to
 * manually assure that the pr_jid field falls on an eight byte boundary.
 * It doesn't naturally fall there, so we fiddle with the pad field to
 * get it there.  Woe unto anyone who changes this.
 */
	int32_t	pr_fill1[1];	/* spares */
	jid_t	pr_jid;		/* job id */
	int32_t	pr_fill[5];	/* spares */
#endif
} prpsinfo_t;
#if _MIPS_SIM != _ABI64
#pragma pack(0)
...
#define PRMAPMAX	16384	/* Upper limit on returned memory mappings */
