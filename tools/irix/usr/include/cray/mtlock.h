...
#elif   defined(_LITTLE_ENDIAN) && !defined(__sv2)
/* ok to use the following locks 
#define MEM_LOCK(lock)          pthread_mutex_lock(lock) 
#define MEM_UNLOCK(lock)        pthread_mutex_unlock(lock) 
*/
extern int c_n_s_(plock_t *lock, long const1, long const2);
#define MEM_LOCK(lock)  	{ while (! c_n_s_(lock,ZZZero,OOOne)); }
#define MEM_UNLOCK(lock) 	{ c_n_s_(lock,OOOne,ZZZero); }
