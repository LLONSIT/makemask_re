...
#define _POSIX93 \
		((defined(_POSIX_C_SOURCE) && (_POSIX_C_SOURCE+0 >= 199309L)) ||\
		 (defined(_SGI_SOURCE) && _NO_XOPEN4 && _NO_XOPEN5 && _NO_POSIX) || \
		 (_ANSIMODE && _NO_XOPEN4 && _NO_XOPEN5 && _NO_POSIX))
...
#define _XOPEN4UX \
		...
		(defined(_SGI_SOURCE) && _NO_POSIX && _NO_XOPEN4 && _NO_XOPEN5) || \
...
/*
 * predicate for XNS5.2 additions -
 * mode: ANSI	-> symbols visible
 *	 POSIX	-> symbols not visible
 *	 SGI	-> symbols visible
 *	 XPG4	-> symbols visible
 *	 XPG5	-> symbols visible
 */
#define _XOPEN5_2	((defined(_XOPEN_SOURCE) && \
				(_XOPEN_SOURCE+0 >= 520)) || \
			((defined(_SGI_SOURCE) && _NO_POSIX && \
				_NO_XOPEN4 && _NO_XOPEN5)) || \
			(_ANSIMODE && _NO_POSIX))

#define _NO_XOPEN5_2	(!defined(_XOPEN_SOURCE) || \
				(defined(_XOPEN_SOURCE) && \
				(_XOPEN_SOURCE+0 < 520)))
