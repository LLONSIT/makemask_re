...
#if defined(__mips) || defined(__ia64)
#include <sys/wait.h>
#if defined (__ia64)  && !defined(__timespec_defined)
...
#endif
#endif
/* types/vars/funcs for debug tracing */
...
extern void dump_func_trace_info (trace_type trace, char *func_name, char *info, int lineno, char *fn);
#define TRACE(A,B,C) dump_func_trace_info(A, B, C, __LINE__, __FILE__);
#else
#define TRACE(A,B,C)
#endif
...
#if	defined(_WORD32) || defined(__mips) || defined(__ia64)
	int		sw_user:32;	/* For user layer storage */
#else
	int		sw_user:48;	/* For user layer storage */
#endif
#if	defined(_ADDR64) || defined(__mips) || defined(__ia64)
...
#elif	defined(__ia64) && (defined(_LP64) || defined(__LP64__))
typedef long bitptr;
#else
typedef long bitptr;
#endif
 ...
 */
#if	(defined(__ia64) && (defined(_LP64) || defined(__LP64__)))
 ...
 */
#if	defined(DEBUG) && !defined(__mips) && !defined(__ia64)
 ...
 */
#if	defined(__mips) || defined(_SOLARIS) || defined(__ia64)
...
#define _MOVEM(to_b, frm_a, nbits)					\
			...
			if (((M_aoff & 07) | (M_boff & 07) |		\
					(nbits & 07)) == 0 )		\
				{					\
		fastbcopy(BPTR2CP(frm_a), BPTR2CP(to_b), (nbits)/8);    \
				}					\
			else						\
				{					\
				int64		M_Bits;			\
 ...
 * \-+-+-----+--------+----+----+--------------------+--------------------/
 */
#if	defined(_CRAY) || defined(__mips) || defined(__ia64)
/*
   #ifndef	uint64
...
#ifdef linux
        __u32 d_mem:32;         /* data buffer memory alignment; 0 == no alignment */
        __u32 d_miniosz:32;     /* min transfer size; 0 == no min transfer size    */
        __u32 d_maxiosz:32;     /* max transfer size; 0 == no max transfer size    */
        __u32 unused__u32:32;   /* not used                     */
#endif
/*
 *	Function pointers (!)
 */
 ...
 *	use by default.
 */
#if	defined(__mips) || defined(_CRAY) || defined(__ia64)
