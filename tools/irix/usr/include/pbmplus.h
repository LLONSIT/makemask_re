...
#if sgi
/*
 * pm_error() calls exit() after printing an error message.  This makes it
 * very difficult to write an application that doesn't die at the first sign
 * of trouble.  In order to circumvent this, the message output and the call
 * to exit() have been #ifdef'ed out and replaced with an error latch.  The
 * error latch can be queried and reset with calls to pm_errorStatus() and
 * pm_resetError(), respectively.
 *
 * What we should really do is carefully go through every call to pm_error()
 * and replace it with a call to pm_message() and appropriate recovery logic.
 * And pm_message() should also be set up so that a callback to a user
 * defined error handler could be configured.  But that would also require
 * that a real file handle be used rather than stdio's FILE.  All in all it's
 * pretty much a mess and not worth the effort, so we'll just lobotomize
 * pm_error().
 *
 * Note that pm_usage() and pm_perror() also have the same problem.  However,
 * pm_usage isn't called by anything in the library and pm_perror() is only
 * called by pm_openr() and pm_openw().  pm_openw() isn't called by anything
 * in the library and pm_openr() is only called by pbm_loadfont() and
 * pbm_loadpbmfont() which are again, finally, not called by anything in the
 * library.
 */
int pm_errorStatus();
void pm_resetError();
